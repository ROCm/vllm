...
Validates ``torch.ops._rocm_C.rdna35_causal_mha_attn`` against an explicit fp32
...
specific attention because neither is reachable from the shape grid alone:

* **num_seqs > 1**, with the sequences given DIFFERENT lengths -- equal ones
  would still pass if a batch collapsed onto one sequence's Q.
* **head counts whose tuned KV segment count is not a power of two** (40, 48,
  56 give 6, 5, 4). Hq=40 at head_size 128 is Llama-2-13B, Qwen-14B, OPT-13B
  and Baichuan-13B.

Run ``pytest tests/kernels/attention/test_rdna35_causal_mha_attn.py``.
"""
...
try:
    ...
    _have_op = hasattr(torch.ops._rocm_C, "rdna35_causal_mha_attn")
except Exception:
    _have_op = False
if not _have_op:
    pytest.skip("_rocm_C.rdna35_causal_mha_attn not available", allow_module_level=True)
...
from vllm.v1.attention.ops import rdna35_causal_mha_attn  # noqa: E402
...
def _tuned_nseg(head_size: int, num_kv_heads: int) -> int:
    return rdna35_causal_mha_attn.tuned_num_kv_segments(1, head_size, num_kv_heads)
...
def test_non_power_of_two_segments(num_heads, head_size, m):
    ...
    The reduce takes the count at runtime precisely so these run; a dispatch
    that switched over instantiated counts would have to reject them.
    """
    nseg = _tuned_nseg(head_size, num_heads)
...
def test_batched_unequal_lengths(m, num_seqs):
    ...
    sequence's Q, so the lengths differ.
    """
...
def test_rejects_strided_partials():
    ...
    index the partials with raw pointer arithmetic that would read the wrong
    elements silently. This pins the guard.
    """
    dev = "cuda"
    num_heads, head_size = 32, 128
    nseg = _tuned_nseg(head_size, num_heads)
...
def test_predicate_rejects_unsupported():
    ...
    base = dict(
        ...
        max_query_len=1,
        dtype=torch.float16,
        kv_quant_mode=rdna35_causal_mha_attn.KVQuantMode.NONE,
        alibi_slopes=None,
        sinks=None,
        sliding_window=None,
        output_scale=None,
    ...
    assert rdna35_causal_mha_attn.can_run(**base)
    ...
    for override in (
        ...
        assert not rdna35_causal_mha_attn.can_run(**{**base, **override}), override

    # Query lengths 2 and 3 are padded up to 4 rather than rejected.
    for qlen in (2, 3, 4):
        assert rdna35_causal_mha_attn.can_run(**{**base, "max_query_len": qlen})
    ...
    for window in (1, 2, 1024):
        assert not rdna35_causal_mha_attn.can_run(
            **{**base, "sliding_window": window}
        ), window

    # head_size 64 with 32 heads is the one shape the kernel does not beat
    # Triton's 2D path at M=1, but it is routed here anyway rather than carved
    # out -- so it must be accepted, at every query length.
    for qlen in (1, 2, 3, 4):
        assert rdna35_causal_mha_attn.can_run(
            **{
                **base,
                "num_heads": 32,
                "num_kv_heads": 32,
