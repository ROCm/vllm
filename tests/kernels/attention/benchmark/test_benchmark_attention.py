...
Directory Structure:
    ...
    └── tests/kernels/attention/benchmark/
        ...
        ├── conftest.py                     (pytest config for --bench-intermittent)
        ├── goldens/<gfx_target>/
...
Skip/Intermittent Cases:
    ...
    - Intermittent: Cases that run but may be flaky
      ...
      → Use --attn-bench-intermittent pytest flag to enable validation
...
def get_benchmark_script_path() -> Path:
    """
    Get absolute path to attention benchmark.py script
    """
...
def get_config_path(config_name: str) -> Path:
    """
    Get absolute path to benchmark config YAML.
...
def run_benchmark_subprocess(
    ...
    Returns:
        ...
        - json_path: Absolute path to output JSON
    ...
    cmd = [
        ...
        "10",  # 10 second cooldown for thermal management
    ]
...
def test_benchmark_regression(
    ...
    Validation Rules:
        ...
        - Intermittent entries validated only if --attn-bench-intermittent flag set
    ...
    Args:
        ...
        - bench_intermittent: Whether to validate intermittent cases
          (from --attn-bench-intermittent)
    ...
    for key in sorted(matching_keys):
        ...
        if golden_mean > 0:
            ...
            if pct_change > REGRESSION_ERROR_THRESHOLD:
                # Regression (slower)
                num_regressions += 1
                ...
                print(f"Change:   {pct_change * 100:+.2f}% (slower)\n")
            elif pct_change < -REGRESSION_ERROR_THRESHOLD:
                # Speedup (faster)
                num_speedups += 1
                print(f"PERFORMANCE SPEEDUP: {format_config_details(actual)}")
                print(f"Expected: {golden_mean:.6f}s")
                print(f"Actual:   {actual_mean:.6f}s")
                print(f"Change:   {pct_change * 100:+.2f}% (faster)\n")
            else:
                # Within threshold
                num_passed += 1
    ...
    if num_skipped_intermittent > 0:
        print(
            ...
            f"use --attn-bench-intermittent to validate)"
        )
