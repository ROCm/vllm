...
def bronze_builds(df, path, testing):
   if not os.path.exists(path):
      ...
      if update_check:
         ...
         if not testing:
            bronze.to_parquet(path, index=False) 
         else:
            print('final DF:')
            #display(bronze)
...
def bronze_agents(df, path, testing):
   if not os.path.exists(path):
...
def bronze_jobs_ids_for_logs(df, jobs_bronze_path, job_logs_bronze_path, token, org_slug, pipe_slug, testing):
   if not os.path.exists(jobs_bronze_path):
      ...
      if os.path.exists(job_logs_bronze_path):
         ...
         new_job_logurls = job_logurls.copy()
