...
class CUDAGraphMode(enum.Enum):
    ...
    def decode_mode(self) -> "CUDAGraphMode":
        return CUDAGraphMode(
            self.value[0]) if self.separate_routine() else self

    def mixed_mode(self) -> "CUDAGraphMode":
        return CUDAGraphMode(
            self.value[1]) if self.separate_routine() else self
    ...
    def max_cudagraph_mode(self) -> "CUDAGraphMode":
        return (CUDAGraphMode(max(self.value))
                if self.separate_routine() else self)
...
class CompilationConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if ("+rms_norm" not in self.custom_ops
                and "-rms_norm" not in self.custom_ops):
            self.custom_ops.append("+rms_norm")
        if ("+silu_and_mul" not in self.custom_ops
                and "-silu_and_mul" not in self.custom_ops):
            self.custom_ops.append("+silu_and_mul")
        if ("+quant_fp8" not in self.custom_ops
                and "-quant_fp8" not in self.custom_ops):
    ...
    def is_attention_compiled_piecewise(self) -> bool:
        ...
        return (use_fx_graph_piecewise_compilation
                or use_inductor_piecewise_compilation)
        ...
        for op in self.custom_ops:
            ...
            if op_name not in all_ops_in_model:
                ...
                missing_str = ("doesn't exist (or wasn't imported/registered)"
                               if op_name not in CustomOp.op_registry else
