...
if TYPE_CHECKING:
    ...
    VLLM_W4A16_PREFILL_BF16: bool = False
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # ROCm W4A16 linear: cache a dequantized bf16/fp16 copy of each weight at
    # load time so the prefill GEMM (batch M > skinny threshold) runs a dense
    # hipBLASLt GEMM instead of the fused in-kernel int4 unpack. Decode still
    # uses the int4 weights. Trades VRAM (~4x the int4 weight) for prefill
    # compute. The bf16 copy is only kept while there is room left in the
    # gpu_memory_utilization budget (budget = total * util): a weight is cached
    # if (free_mem - total * (1 - util)) >= bf16_bytes at load time, otherwise it
    # keeps the int4 prefill path. The check is self-limiting -- each copy
    # shrinks the spendable budget, so later layers stop being cached once the
    # budget is exhausted (a message is emitted). Raise gpu_memory_utilization to
    # cache more weights at the cost of KV-cache memory.
    "VLLM_W4A16_PREFILL_BF16": lambda: (
        os.getenv("VLLM_W4A16_PREFILL_BF16", "false").lower() in ("true", "1")
    ),
