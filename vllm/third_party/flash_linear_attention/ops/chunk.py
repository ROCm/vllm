...
def chunk_gated_delta_rule(
    ...
    if scale is None:
        scale = k.shape[-1] ** -0.5
    # Annotate the forward with the FLA chunk size so a profiler trace carries
    # the un-derivable chunk size to roofline tooling.  record_function under
    # profiling, nullcontext otherwise (zero overhead).
    with record_function_or_nullcontext(f"gdn_chunk_fwd chunk_size={FLA_CHUNK_SIZE}"):
