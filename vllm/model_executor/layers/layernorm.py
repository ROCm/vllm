...
def rocm_aiter_fused_rms_fp8_group_quant_impl(
        x: torch.Tensor, residual: torch.Tensor, weight: torch.Tensor,
        variance_epsilon: float) -> tuple[torch.Tensor, torch.Tensor]:
...
def rocm_aiter_fused_rms_fp8_group_quant_fake(
        x: torch.Tensor, residual: torch.Tensor, weight: torch.Tensor,
        variance_epsilon: float) -> tuple[torch.Tensor, torch.Tensor]:
    M, N = x.shape
...
if current_platform.is_rocm():
    ...
    direct_register_custom_op(
        op_name="rocm_aiter_fused_rms_fp8_group_quant",
        op_func=rocm_aiter_fused_rms_fp8_group_quant_impl,
        mutates_args=[],
        fake_impl=rocm_aiter_fused_rms_fp8_group_quant_fake,
        dispatch_key=current_platform.dispatch_key,
    )
