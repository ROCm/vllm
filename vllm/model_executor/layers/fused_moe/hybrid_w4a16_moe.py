...
class HybridW4A16MoEExperts(mk.FusedMoEExpertsModular):
    ...
    # neutral (not measured in this PR).  Gated behind
    # VLLM_HYBRID_W4A16_TRITON_TUNED_P1=1 (default ON, name kept for
    # backwards compat with the env var introduced by the earlier tune).
    _P1_TUNED = os.environ.get("VLLM_HYBRID_W4A16_TRITON_TUNED_P1", "1") == "1"
    TRITON_BLOCK_SIZE_M = 32 if _P1_TUNED else 64
    ...
    def _triton_config(
        ...
        evidence.  Gated behind VLLM_HYBRID_W4A16_TRITON_TUNED_P1 (env
        name kept for back-compat; default ON).
        ...
        if HybridW4A16MoEExperts._P1_TUNED:
            ...
            if K < 1024:
                ...
                # tuned-P1 (BLOCK_M=128, BN=32, BK=group, GM=8, nw=4,
                # ns=1) baseline across all routing densities (conc_16
            ...
            # densities at NUM_TOKENS=128.
            return {
                "BLOCK_SIZE_M": self.TRITON_BLOCK_SIZE_M,  # 32 when tuned
                "BLOCK_SIZE_N": 16,
    ...
    def apply(
        ...
        if scattered and expert_map is None:
            ...
            # topk_ids is freshly allocated each call by fused_topk; under
            # CUDA graph capture PyTorch's allocator returns a stable address
            # for the same allocation, so we can pass the view directly and
            # avoid an extra device-to-device copy into a cached buffer.
