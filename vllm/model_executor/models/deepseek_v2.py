...
def _indexer_k_quant_and_cache_kernel(
    ...
    kv_cache_ptr,       # [num_blocks, block_size, head_dim] -> [num_blocks, block_size // 16 (tile_block_size), head_dim // 16byte, 16 (tile_block_stride), 16byte]
    kv_cache_scale_ptr, # [num_blocks, block_size]
    ...
    tile_block_size = block_size // BLOCK_TILE_SIZE
    val = tl.load(src_ptr + offset)
    amax = tl.max(val.abs(), axis=-1).to(tl.float32)
    if IS_FNUZ:
        scale = tl.maximum(1e-4, amax) / 224.
    else:
        scale = tl.maximum(1e-4, amax) / 448.

    if USE_UE8M0:
        scale = tl.exp2(tl.ceil(tl.log2(scale)))

    fp8_val = (val.to(tl.float32) / scale).to(kv_cache_ptr.type.element_ty)
    dst_ptr = kv_cache_ptr + block_id * kv_cache_value_stride + tile_block_id * BLOCK_TILE_SIZE * head_dim + tile_block_offset * HEAD_TILE_SIZE
...
def _cp_gather_indexer_quant_cache_kernel(
    ...
    tiled_block_size = block_size // BLOCK_TILE_SIZE
    tiled_block_id = block_offset // BLOCK_TILE_SIZE
    tiled_block_offset = block_offset % BLOCK_TILE_SIZE
    src_cache_offset = block_id * kv_cache_stride + tiled_block_id * HEAD_DIM * BLOCK_TILE_SIZE + tiled_block_offset * HEAD_TILE_SIZE
    ...
    if has_prefill:
        ...
        for chunk in prefill_metadata.chunks:
            ...
            cp_gather_indexer_k_quant_cache_triton(
                kv_cache, k_fp8, k_scale, chunk.block_table, chunk.cu_seq_lens, chunk.token_to_seq,
            )
            # torch.cuda.synchronize()
