...
class ModelArchConfigConvertorBase:
    ...
    def get_num_experts_per_token(self) -> int:
        """Returns the number of routed experts selected per token."""
        return (
            getattr(self.hf_text_config, "num_experts_per_tok", 0)
            or getattr(self.hf_text_config, "num_selected_experts", 0)
            or getattr(self.hf_text_config, "num_experts_per_token", 0)
        )

    def rswa_window(self) -> int | None:
        """Returns the Reference Sliding Window Attention window size."""
        return getattr(self.hf_text_config, "rswa_window", None)
...
class Gemma4ModelArchConfigConvertor(ModelArchConfigConvertorBase):
    ...
    def get_total_num_kv_heads(self) -> int:
        """Handle heterogeneous Gemma4 configs from Transformers v5.

        Gemma4 uses more KV heads on full-attention layers when attention_k_eq_v
        is enabled. Transformers v5 heterogeneous configs forbid direct access to
        config.num_key_value_heads; must read from per_layer_config instead.
        """
        text_config = self.hf_text_config

        # Transformers >= 5.15.0: heterogeneous config with per_layer_config
        if getattr(text_config, "is_heterogeneous", False):
            per_layer_configs = getattr(text_config, "per_layer_config", [])
            if per_layer_configs:
                # Return MAX kv heads across all layers (needed for KV cache sizing)
                return max(
                    getattr(layer_cfg, "num_key_value_heads", 0)
                    for layer_cfg in per_layer_configs
                )

        # Transformers < 5.15.0: old-style with layer_types + flat attributes
        if getattr(text_config, "layer_types", None):
            num_kv_heads = getattr(text_config, "num_key_value_heads", None)
            # Check if full_attention layers use more KV heads
            if getattr(text_config, "attention_k_eq_v", False):
                global_kv_heads = getattr(
                    text_config, "num_global_key_value_heads", None
                )
                if global_kv_heads and num_kv_heads:
                    return max(num_kv_heads, global_kv_heads)
                elif global_kv_heads:
                    return global_kv_heads
            if num_kv_heads:
                return num_kv_heads

        # Fallback: homogeneous config or no KV head variation
        return getattr(
            text_config,
            "num_key_value_heads",
