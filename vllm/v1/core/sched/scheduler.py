...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        if envs.VLLM_BATCH_INVARIANT and self.scheduler_config.enable_chunked_prefill:
            ...
            # Not covered: multimodal requests. _try_schedule_encoder_inputs can
            # shrink num_new_tokens again below, from a per-step encoder budget
            # and a shared encoder cache, neither of which is a function of the
            # request alone. It only binds when the capped chunk is shorter than
            # one multimodal item's embeddings; when it does, the chunk is cut
            # back to the item's offset and the MLA split moves with it.
