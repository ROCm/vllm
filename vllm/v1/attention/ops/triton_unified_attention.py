...
# (num_kv_heads, head_size), as (num_warps, num_stages, waves_per_eu,
# tile_size).  A tile_size of None keeps the adaptive _get_tile_size result.
# Shapes absent from the table keep the generic assignment above.
_GFX1151_3D_DECODE = {
    (2, 64): (2, 1, 2, 32),
    (2, 128): (4, 1, 1, 64),
    (2, 512): (4, 1, 2, None),
...
# Windowed layers need their own table: the window bounds the KV traversed,
# so the optimum shifts towards more split-K and the dense pick is 27-88% off.
...
def _get_tile_size(
    ...
    """Select tile size with Gemma3-specific optimization and ROCm tuning."""
    ...
    # ROCm Navi optimization: Use power-of-2 of block_size
...
def unified_attention(
    ...
    if launch_num_warps is not None:
        config["num_warps"] = launch_num_warps
    if launch_num_stages is not None:
        config["num_stages"] = launch_num_stages
