...
def run_sweep(shapes, block_sizes, warmup, rep, dtype, csv_path):
    ...
    for M, K, label in shapes:
        ...
        for bsm in block_sizes:
            ...
            # Production anchor — dispatcher chooses ytile/unrl.  Bind
            # loop-captured tensors as default args (ruff B023).
            prod_us = time_us(
                lambda a=a, w=w, scales=scales, c=c, eids=expert_ids, bsm=bsm: (
                    ops.fused_moe_wvSplitK_int4_gemm(
                        a,
                        w,
                        scales,
                        c,
            ...
            for yt, un, ac, wv in itertools.product(YTILES, UNRLS, ACHUNKS, WVPRGRPS):
                ...
                try:
                    ...
                    print(
                        f"  ERROR bsm={bsm} {M}x{K} yt={yt} un={un} "
                        f"ac={ac} wv={wv}: {e}"
                    )
                    us, bw = float("inf"), 0.0
                writer.writerow(
