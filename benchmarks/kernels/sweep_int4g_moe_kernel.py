...
def build_moe_tensors(M, K, block_size_m, dtype, device):
    ...
    w_int8 = torch.randint(
        -8, 8, (NUM_EXPERTS, M, K), dtype=torch.int8, device=device
    )
    w_packed_i8 = torch.stack(
        [pack_int4(w_int8[e]) for e in range(NUM_EXPERTS)], dim=0
    )
...
def run_sweep(shapes, block_sizes, warmup, rep, dtype, csv_path):
    ...
    if "_rocm_C::fused_moe_wvSplitK_int4_gemm_sweep" not in torch._C._dispatch_get_all_op_names():
        raise SystemExit(
    ...
    print(f"Grid: YTILE={YTILES} x UNRL={UNRLS} x A_CHUNK={ACHUNKS} x WvPrGrp={WVPRGRPS}")
    ...
    writer.writerow([
        "M", "K", "block_size_m", "label",
        "ytile", "unrl", "achunk", "wvprgrp",
        "time_us", "weight_bw_gibs", "is_production_pick",
    ])
    ...
    for M, K, label in shapes:
        ...
        for bsm in block_sizes:
            ...
            prod_us = time_us(
                lambda: ops.fused_moe_wvSplitK_int4_gemm(
            ...
            for yt, un, ac, wv in itertools.product(YTILES, UNRLS, ACHUNKS, WVPRGRPS):
                ...
                try:
                    us = time_us(
                        lambda yt=yt, un=un, ac=ac, wv=wv:
                            ops.fused_moe_wvSplitK_int4_gemm_sweep(
                                a, w, scales, c, expert_ids,
                                bsm, cu_count, GROUP_SIZE,
                                yt, un, ac, wv,
                            ),
                        warmup=warmup, rep=rep,
                    )
                    ...
                    print(
                        f"  ERROR bsm={bsm} {M}x{K} yt={yt} un={un} ac={ac} wv={wv}: {e}"
                    )
                    us, bw = float("inf"), 0.0
                writer.writerow([M, K, bsm, label, yt, un, ac, wv,
                                 f"{us:.1f}", f"{bw:.1f}", ""])
                shape_rows.append((yt, un, ac, wv, us, bw))
                tested += 1

            # Production anchor row.
            writer.writerow([M, K, bsm, label, "", "", "", "",
            ...
            if shape_rows:
                ...
                best_per_shape.append({
                    "M": M, "K": K, "bsm": bsm, "label": label,
                    "ytile": best[0], "unrl": best[1],
                    "achunk": best[2], "wvprgrp": best[3],
                    "time_us": best[4], "bw_gibs": best[5],
                    "prod_us": prod_us,
                })
    ...
    print("=" * 120)
    print("BEST CONFIG PER SHAPE  (fused_moe_sweep best vs production fused_moe_wvSplitK_int4_gemm)")
    print("=" * 120)
    print(f"{'bsm':>3} {'M':>5}x{'K':<5}  {'Label':<48}  "
          f"{'yt':>3} {'un':>3} {'ac':>3} {'wv':>3}  {'best_us':>9}  {'prod_us':>9}  {'speedup':>8}")
    print("-" * 120)
    for r in best_per_shape:
        speedup = r["prod_us"] / r["time_us"] if r["time_us"] > 0 else 0.0
        print(f"{r['bsm']:>3} {r['M']:>5}x{r['K']:<5}  {r['label']:<48}  "
...
def main():
    ...
    ap.add_argument("--block-sizes", type=int, nargs="+", default=[1],
