...
__device__ __forceinline__ bool moe_sum_pad_aware_skip(
    ...
    int32_t num_experts, int64_t idx) {
...
__global__ void moe_sum_vec_kernel(
    ...
    const int32_t* __restrict__ expert_map, const int32_t num_experts,
...
#pragma unroll
    for (int k = 0; k < TOPK; ++k) {
      if constexpr (PAD_AWARE) {
        if (moe_sum_pad_aware_skip(tk_tok, expert_map, num_experts,
                                   k * stride_tk_k)) {
          continue;
        }
      }
...
__global__ void moe_sum_vec_dynamic_kernel(
    ...
    const idx_t* __restrict__ topk_ids, const int32_t* __restrict__ expert_map,
    const int32_t num_experts, const int64_t stride_tk_token,
...
void moe_sum(torch::stable::Tensor& input,   // [num_tokens, topk, hidden_size]
  ...
  if (topk_ids.has_value()) {
    ...
    if (expert_map.has_value()) {
      ...
      // `expert_map` is indexed by GLOBAL expert id, so its length is the
      // bound `moe_sum_pad_aware_skip` must enforce.
      expert_map_len = static_cast<int32_t>(expert_map->numel());
    }
...
#define LAUNCH_MOE_SUM_PAD_AWARE_VEC(TOPK)                                     \
  vllm::moe::moe_sum_vec_kernel<scalar_t, idx_t, TOPK, true>                   \
      <<<grid, dim3(block), 0, stream>>>(                                      \
          ...
          topk_ids_ptr, expert_map_ptr, expert_map_len, stride_tk_token,       \
          stride_tk_k)
                    ...
                    default:
                      vllm::moe::moe_sum_vec_dynamic_kernel<scalar_t, idx_t,
                          ...
                          <<<grid, dim3(block), 0, stream>>>(
                              ...
                              expert_map_ptr, expert_map_len, stride_tk_token,
                              stride_tk_k);
                      break;
                  }
                } else {
