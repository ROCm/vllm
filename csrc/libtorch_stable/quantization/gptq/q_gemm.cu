...
void fused_moe_exllama_gemm(
    torch::stable::Tensor a,              // [num_tokens, K] fp16 activations
    torch::stable::Tensor b_q_weight,     // [E, K/8, N] int32 packed weights
    torch::stable::Tensor b_gptq_qzeros,  // [E, groups, N/8] int32 packed zeros
    torch::stable::Tensor b_gptq_scales,  // [E, groups, N] fp16 scales
    torch::stable::Tensor c,  // [M*topk, N] fp16 output (pre-allocated)
    torch::stable::Tensor sorted_token_ids,  // [max_num_tokens_padded] int32
    torch::stable::Tensor expert_ids,        // [num_blocks] int32
    torch::stable::Tensor topk_weights,      // [M*topk] float32
...
#define LAUNCH_CONTIGUOUS(M_COUNT)                                            \
  vllm::gptq::moe_gptq_4bit_contiguous_kernel<M_COUNT>                        \
    ...
    if (block_size_m == 2)
      ...
      STD_TORCH_CHECK(
          false,
          "Unsupported block_size_m for contiguous MoE kernel: ", block_size_m);
