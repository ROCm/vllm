...
struct BaseGemmPipelineAgBgCrCompV3_bias
{
...
template <typename Problem, typename Policy = UniversalGemmPipelineAgBgCrPolicy_bias>
struct GemmPipelineAgBgCrCompV3_bias : public BaseGemmPipelineAgBgCrCompV3_bias<Problem>
{
    using Base             = BaseGemmPipelineAgBgCrCompV3_bias<Problem>;
    ...
    struct PipelineImpl<GemmPipelineScheduler::Intrawave> : public PipelineImplBase
    {
                                       ...
                                       void* p_smem) const
        {
            ...
            // __builtin_amdgcn_sched_barrier(0);
            // constexpr auto s0acc_spans = decltype(c_block_tile)::get_distributed_spans();
            // sweep_tile_span(s0acc_spans[number<0>{}], [&](auto idx0) {
            //     sweep_tile_span(s0acc_spans[number<1>{}], [&](auto idx1) {
            //         constexpr auto i_j_idx = make_tuple(idx0, idx1);
            //         const auto val         = c_block_tile[i_j_idx];
     ...
     */
    template <typename ADramBlockWindowTmp, typename BDramBlockWindowTmp,typename BiasDramBlockWindowTmp >
    CK_TILE_DEVICE auto operator()(const ADramBlockWindowTmp& a_dram_block_window_tmp,
                                   const BDramBlockWindowTmp& b_dram_block_window_tmp,
                                   const BiasDramBlockWindowTmp& bias_dram_block_window_tmp,
                                   index_t num_loop,
