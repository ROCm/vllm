// Per-N kernel instantiation shard for wvSplitK_int4 (N=5).

#include "dispatch.cuh"
#include "launch.h"

template <typename scalar_t>
void launch_int4_n5(dim3 grid, cudaStream_t stream, int K_in, int M_in,
...
template void launch_int4_n5<half>(dim3, cudaStream_t, int, int, int, int,
                                   const uint8_t*, const half*, const half*,
                                   const half*, const half*, half*, int, int,
                                   int64_t, int, bool);
template void launch_int4_n5<__hip_bfloat16>(
...
template <typename scalar_t>
void launch_moe_int4_n5(dim3 grid, cudaStream_t stream, int K_in, int M_in,
  ...
  dispatch_moe_int4_g<scalar_t, 5>(
...
template void launch_moe_int4_n5<half>(dim3, cudaStream_t, int, int, int,
                                       const uint8_t*, const half*, const half*,
                                       const half*, half*, const int*,
                                       const int*, int, long, long, long, int,
                                       int, int64_t, int, bool, bool);
template void launch_moe_int4_n5<__hip_bfloat16>(
