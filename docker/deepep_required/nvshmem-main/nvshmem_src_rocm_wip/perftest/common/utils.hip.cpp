...
void init_cumodule(const char *str) {
    ...
    CU_CHECK(hipModuleLoad(&mymodule, cubin_path));
...
void init_test_case_kernel(hipFunction_t *kernel, const char *kernel_name) {
    CU_CHECK(hipModuleGetFunction(kernel, mymodule, kernel_name));
}
...
void alloc_tables(void ***table_mem, int num_tables, int num_entries_per_table) {
    ...
    CUDA_CHECK(
        hipDeviceGetAttribute(&dev_property, hipDeviceAttributeUnifiedAddressing, mype_node % dev_count));
    ...
    for (i = 0; i < num_tables; i++) {
        CUDA_CHECK(
            hipHostAlloc(&tables[i], num_entries_per_table * sizeof(double), hipHostMallocMapped));
...
void datatype_parse(char *optarg, datatype_t *datatype) {
    if (!strcmp(optarg, "int")) {
        ...
        datatype->size = sizeof(__hip_bfloat16);
        datatype->name = "bf16";
    }
}
