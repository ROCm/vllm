...
#define DEFINE_ATOMIC_LATENCY_PING_PONG_CALL_KERNEL(AMO, TYPE_NAME)                            \
    void test_ping_pong_##TYPE_NAME##_##AMO##_cubin(hipStream_t stream, void **arglist) {     \
        hipFunction_t test_cubin;                                                                 \
        init_test_case_kernel(&test_cubin,                                                     \
                              NVSHMEMI_TEST_STRINGIFY(ping_pong_##TYPE_NAME##_##AMO));         \
        CU_CHECK(hipModuleLaunchCooperativeKernel(test_cubin, 1, 1, 1, 1, 1, 1, 0, stream, arglist)); \
    }
...
#define RUN_TEST_WITHOUT_ARG(TYPE, TYPE_NAME, AMO, flag_d, mype, iter, skip, h_lat, h_size_arr, \
    ...
    do {                                                                                        \
        ...
        TYPE flag_init_var = flag_init;                                                         \
                                                                                                \
        CUDA_CHECK(hipDeviceSynchronize());                                                    \
        CUDA_CHECK(hipMemcpy(flag_d, &flag_init_var, sizeof(TYPE), hipMemcpyHostToDevice));   \
        nvshmem_barrier_all();                                                                  \
                                                                                                \
