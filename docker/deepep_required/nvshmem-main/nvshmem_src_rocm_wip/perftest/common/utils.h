...
#define CUMODULE_LOAD(CUMODULE, CUMODULE_PATH, ERROR) \
    CU_CHECK(hipModuleLoad(&CUMODULE, CUMODULE_PATH)); \
...
#define CU_CHECK(stmt)                                                                     \
    do {                                                                                   \
        ...
        char str[1024];                                                                    \
        if (hipSuccess != result) {                                                      \
            hipError_t ret = hipDrvGetErrorString(result, (const char **)&str);                  \
            fprintf(stderr, "[%s:%d] cuda failed with (%d) %s \n", __FILE__, __LINE__,     \
                    (int)result, (ret != hipSuccess) ? "hipDrvGetErrorString failed" : str); \
            exit(-1);                                                                      \
        }                                                                                  \
        assert(hipSuccess == result);                                                    \
    } while (0)
