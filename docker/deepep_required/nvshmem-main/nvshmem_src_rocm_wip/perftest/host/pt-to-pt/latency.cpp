...
int lat(void *data_d, void *data_d_local, int sizeBytes, int pe, int iter, int skip,
        putget_issue_t iss, dir_t dir, hipStream_t strm, hipEvent_t sev, hipEvent_t eev,
        float *ms, float *us) {
    int status = 0;
    int peer = !pe;
    ...
    if (iss.type == ON_STREAM) {
        if (dir.type == WRITE) {
            for (int i = 0; i < (iter + skip); i++) {
                if (i == skip) CUDA_CHECK(hipEventRecord(sev, strm));
        ...
        CUDA_CHECK(hipEventElapsedTime(ms, sev, eev));
    } else {
        if (dir.type == WRITE) {
            for (int i = 0; i < (iter + skip); i++) {
...
int main(int argc, char *argv[]) {
    ...
    CUDA_CHECK(hipMalloc((void **)&data_d_local, max_size));
#else
    data_d_local = (char *)nvshmem_malloc(max_size);
#endif
    CUDA_CHECK(hipMemset(data_d_local, 0, max_size));
...
#ifdef _NVSHMEM_REGISTRATION_CACHE_ENABLED
    if (data_d_local) hipFree(data_d_local);
#else
