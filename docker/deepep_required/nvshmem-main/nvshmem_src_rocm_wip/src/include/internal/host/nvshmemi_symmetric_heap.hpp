 ...
 *      SHM                      PINNED                                VMM
 *
 * Supported memory kinds: sysmem (linux shm), vidmem (hipMalloc), vidmem (hipMemCreate)
 */
...
class nvshmemi_symmetric_heap {
   public:
    ...
    hipMemAllocationHandleType get_mem_handle_type(void) { return mem_handle_type_; }
    bool is_cuda_mem_handle_type_ipc(void) const {
        return (mem_handle_type_ == hipMemHandleTypePosixFileDescriptor);
    }
   ...
   protected:
    ...
    void set_mem_handle_type(hipMemAllocationHandleType type) { mem_handle_type_ = type; }
    ...
    hipMemAllocationHandleType mem_handle_type_ = hipMemHandleTypeNone;
...
class nvshmemi_symmetric_heap_vidmem_dynamic_vmm final
   ...
   protected:
    hipMemGenericAllocationHandle_t get_cumem_handle_ptr(int i) {
   ...
   private:
    void set_cuda_mem_prop(__attribute__((unused)) void *prop, int mem_handle_type) {
        hipMemAllocationProp *memprop = (hipMemAllocationProp *)(prop);
        ...
        (*memprop).requestedHandleType = (hipMemAllocationHandleType)(mem_handle_type);
        (*memprop).allocFlags.gpuDirectRDMACapable = 1;
        return;
    }

    std::vector<std::tuple<hipMemGenericAllocationHandle_t, off_t, off_t, size_t>> cumem_handles_;
};
