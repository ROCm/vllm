...
#include "internal/host_transport/cudawrap.h"                    // for hipMemGenericAllocationHandle_t
...
class nvshmemi_nvls_rsc final {
   public:
    ...
    hipMemGenericAllocationHandle_t *get_mc_handle_ptr(int idx) { return &cumc_handles_[idx].first; }
    ...
    int import_group(char *shareable_handle, hipMemGenericAllocationHandle_t *mc_handle,
                     uint64_t mem_size);
    int subscribe_group(hipMemGenericAllocationHandle_t *mc_handle);
    int reserve_group_mem(void);
    int free_group_mem(void);
    int bind_group_mem(hipMemGenericAllocationHandle_t *mc_handle, nvshmem_mem_handle_t *mem_handle,
                       size_t mem_size, off_t mem_offset, off_t mc_offset);
    int map_group_mem(hipMemGenericAllocationHandle_t *mc_handle, size_t mem_size, off_t mem_offset,
                      off_t mc_offset);
    int unbind_group_mem(hipMemGenericAllocationHandle_t *mc_handle, off_t mc_offset, size_t mem_size);
   ...
   private:
    ...
    //hipMemAllocationHandleType alloc_mem_handle_type_ = CU_MEM_HANDLE_TYPE_NONE;
    hipMemAllocationHandleType alloc_mem_handle_type_;
 ...
 #endif
    hipDevice_t current_dev_;
    std::vector<std::pair<hipMemGenericAllocationHandle_t, uint64_t>> cumc_handles_;
