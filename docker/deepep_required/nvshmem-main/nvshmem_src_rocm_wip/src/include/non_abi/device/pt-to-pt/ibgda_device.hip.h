...
__device__ NVSHMEMI_DEVICE_ALWAYS_INLINE int nvshmemi_thread_id_in_warp() {
    return threadIdx.x % NVSHMEMI_WARP_SIZE;
}
...
__device__ NVSHMEMI_STATIC NVSHMEMI_DEVICE_ALWAYS_INLINE uint32_t ibgda_get_smid() {
    ...
    uint32_t hw_id_ = __builtin_amdgcn_s_getreg(_GETREG_IMMED(31, 0, _HWREG_HW_ID));
    uint32_t smid = (hw_id_ >> 8) & _BM(4);
    return smid;
}
...
#if __HIP__ || CUDART_VERSION >= 12000
