...
#endif
#include "non_abi/device/pt-to-pt/proxy_device.hip.h"
...
template <>
struct is_bfloat<__hip_bfloat16> : std::true_type {};
...
template <>
struct is_bfloat<hip_bfloat16> : cuda::std::true_type {};
...
#pragma unroll
    for (int unroll = 0; unroll < UNROLL; unroll++) {
        if (sizeof(T) >= 8) {
             ...
             * appropriate due to 256 B alignment of hipMalloc().
             */
            uint32_t *src4 =
