...
#include <hip/hip_runtime_api.h>                            // for hipGetErrorString
#include <limits.h>                                  // for INT_MAX, INT_MIN
#include <stdio.h>                                   // for fprintf, stderr
#include <hip/hip_vector_types.h>                            // for dim3
...
static int _nvshmemi_collective_launch_query_gridsize(const void *func, dim3 blockDims, void **args,
    ...
    status =
        hipOccupancyMaxActiveBlocksPerMultiprocessor(&maxBlocksSM, func, blockSize, sharedMem);
...
static int _nvshmemi_collective_launch(const void *func, dim3 gridDims, dim3 blockDims, void **args,
    ...
    status =
        hipOccupancyMaxActiveBlocksPerMultiprocessor(&maxBlocksSM, func, blockSize, sharedMem);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipOccupancyMaxActiveBlocksPerMultiprocessor failed aborting job\n");
    ...
    CUDA_RUNTIME_CHECK_GOTO(
        hipEventRecord(nvshmemi_device_only_state.claunch_params.begin_event, stream), status,
        out);
    CUDA_RUNTIME_CHECK_GOTO(
        hipStreamWaitEvent(nvshmemi_device_only_state.claunch_params.stream,
                            nvshmemi_device_only_state.claunch_params.begin_event, 0),
        status, out);

    if (nvshmemi_device_only_state.cu_dev_attrib.cooperative_launch) {
        status = hipLaunchCooperativeKernel(reinterpret_cast<const void*>(func), gridDims, blockDims, args, sharedMem,
                                             nvshmemi_device_only_state.claunch_params.stream);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_COLLECTIVE_LAUNCH_FAILED, out,
                              "Cooperative kernel launch failed \n");
    } else {
        status = hipLaunchKernel(reinterpret_cast<const void*>(func), gridDims, blockDims, args, sharedMem,
                                  nvshmemi_device_only_state.claunch_params.stream);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_COLLECTIVE_LAUNCH_FAILED, out,
                              "Kernel launch failed \n");
    }

    CUDA_RUNTIME_CHECK_GOTO(hipEventRecord(nvshmemi_device_only_state.claunch_params.end_event,
                                            nvshmemi_device_only_state.claunch_params.stream),
                            status, out);

    CUDA_RUNTIME_CHECK_GOTO(
        hipStreamWaitEvent(stream, nvshmemi_device_only_state.claunch_params.end_event, 0), status,
        out);
...
int nvshmemi_setup_collective_launch() {
    ...
    CUDA_RUNTIME_CHECK_GOTO(
        hipDeviceGetAttribute(&(nvshmemi_device_only_state.cu_dev_attrib.multi_processor_count),
                               hipDeviceAttributeMultiprocessorCount,
                               nvshmemi_device_only_state.cuda_device_id),
        status, out);

    CUDA_RUNTIME_CHECK_GOTO(
        hipDeviceGetAttribute(&(nvshmemi_device_only_state.cu_dev_attrib.cooperative_launch),
    ...
    CUDA_RUNTIME_CHECK_GOTO(hipDeviceGetStreamPriorityRange(&leastPriority, &greatestPriority),
                            status, out);
    CUDA_RUNTIME_CHECK_GOTO(
        hipStreamCreateWithPriority(&nvshmemi_device_only_state.claunch_params.stream,
                                     hipStreamNonBlocking, greatestPriority),
        status, out);
    CUDA_RUNTIME_CHECK_GOTO(hipEventCreateWithFlags(&nvshmemi_device_only_state.claunch_params.begin_event,
                                            hipEventDisableTiming),
                            status, out);
    CUDA_RUNTIME_CHECK_GOTO(hipEventCreateWithFlags(&nvshmemi_device_only_state.claunch_params.end_event,
                                            hipEventDisableTiming),
                            status, out);
...
int nvshmemi_teardown_collective_launch() {
    ...
    CUDA_RUNTIME_CHECK_GOTO(hipStreamDestroy(nvshmemi_device_only_state.claunch_params.stream),
                            status, out);
    CUDA_RUNTIME_CHECK_GOTO(hipEventDestroy(nvshmemi_device_only_state.claunch_params.begin_event),
                            status, out);
    CUDA_RUNTIME_CHECK_GOTO(hipEventDestroy(nvshmemi_device_only_state.claunch_params.end_event),
                            status, out);
