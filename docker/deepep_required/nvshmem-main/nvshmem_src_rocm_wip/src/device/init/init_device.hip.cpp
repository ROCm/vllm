...
void nvshmemi_check_state_and_init_d() {
    ...
    if (nvshmemid_init_status() == NVSHMEM_STATUS_IS_BOOTSTRAPPED) {
        ...
        status = hipGetDevice(&nvshmemi_device_only_state.cuda_device_id);
        if (status) {
...
void nvshmemi_get_mem_handle(void **dev_state_ptr, void **transport_dev_state_ptr) {
    int status = 0;
    status = hipGetSymbolAddress(dev_state_ptr, HIP_SYMBOL(nvshmemi_device_state_d));
    if (status) {
        NVSHMEMI_ERROR_PRINT("Unable to access device state. %d\n", status);
        *dev_state_ptr = NULL;
    }
#ifdef NVSHMEM_IBGDA_SUPPORT
    status = hipGetSymbolAddress(transport_dev_state_ptr, HIP_SYMBOL(nvshmemi_ibgda_device_state_d));
    if (status) {
