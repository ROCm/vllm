...
#include <hip/hip_runtime.h>                                        // for hipGetDevice
...
nvshmemi_nvls_rsc::nvshmemi_nvls_rsc(nvshmemi_team_t *team, nvshmemi_state_t *state) {
    ...
    status = hipDeviceGet(&current_dev_, cuda_dev);
...
void nvshmemi_nvls_rsc::invalidate_rsc(void) {
    ...
    alloc_mem_handle_type_ = hipMemHandleTypeNone;
#if !__HIP__
    prop_ = {};
#endif
    virt_alloc_size_ = 0;
...
int nvshmemi_nvls_rsc::export_group(uint64_t mem_size, char *shareable_handle) {
    ...
    status = cuMulticastGetGranularity(&mc_granularity_, &prop_, CU_MULTICAST_GRANULARITY_RECOMMENDED);
    ...
    status = cuMulticastCreate(&mc_handle, &prop_);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
    ...
    status = hipMemExportToShareableHandle(shareable_handle, mc_handle, alloc_mem_handle_type_, 0);

    if (state->heap_obj->is_cuda_mem_handle_type_fabric()) {
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                              "hipMemExportToShareableHandle failed for fabric handles\n");
        INFO(NVSHMEM_TEAM, "Exporting mcHandle %lld via FH on GPU device %d\n", mc_handle,
             current_dev_);

    } else {
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                              "hipMemExportToShareableHandle failed for ipc handles\n");
...
int nvshmemi_nvls_rsc::import_group(char *shareable_handle, hipMemGenericAllocationHandle_t *mc_handle,
                                    uint64_t mem_size) {
    nvshmemi_state_t *state = get_state();
    int status = -1;
#if !__HIP__ 
    if (state->heap_obj->is_cuda_mem_handle_type_ipc()) {
        ...
        status = hipMemImportFromShareableHandle(mc_handle, (void *)(uintptr_t)fd,
                                                      alloc_mem_handle_type_);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                              "hipMemImportFromShareableHandle failed for ipc handles\n");
        INFO(NVSHMEM_TEAM, "Importing mcHandle %lld via POSIX FD on GPU device %d\n", *mc_handle,
             current_dev_);
        close(fd);
    } else {
        status = hipMemImportFromShareableHandle(mc_handle, (void *)shareable_handle,
                                                      alloc_mem_handle_type_);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                              "hipMemImportFromShareableHandle failed for fabric handles \n");
...
int nvshmemi_nvls_rsc::subscribe_group(hipMemGenericAllocationHandle_t *mc_handle) {
    int status = -1;
#if !__HIP__
    status = cuMulticastAddDevice(*mc_handle, current_dev_);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
...
int nvshmemi_nvls_rsc::reserve_group_mem(void) {
    int status = -1;
#if !__HIP__
    status = hipMemAddressReserve((hipDeviceptr_t *)&mc_base_ptr_, virt_alloc_size_,
                                       alloc_granularity_, (hipDeviceptr_t)NULL, 0);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipMemAddressReserve failed for mc base ptr \n");
...
int nvshmemi_nvls_rsc::free_group_mem(void) {
    ...
    status =
        CUPFN(nvshmemi_cuda_syms, hipMemAddressFree((hipDeviceptr_t)mc_base_ptr_, virt_alloc_size_));
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipMemAddressFree failed for mc base ptr \n");
#endif
out:
    return (status);
}
 ...
 */
int nvshmemi_nvls_rsc::bind_group_mem(hipMemGenericAllocationHandle_t *mc_handle,
                                      nvshmem_mem_handle_t *mem_handle, size_t mem_size,
                                      off_t mem_offset, off_t mc_offset) {
    int status = -1;
#if !__HIP__
    status = cuMulticastBindMem(*mc_handle, mc_offset, *(hipMemGenericAllocationHandle_t *)mem_handle,
                                 mem_offset, mem_size, 0);
    NVSHMEMI_NE_ERROR_JMP(
        status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
    ...
    INFO(NVSHMEM_TEAM,
         ...
         "mc offset %lx\n on GPU device %d\n",
         *mc_handle, *(hipMemGenericAllocationHandle_t *)mem_handle, mem_size, mem_offset, mc_offset,
         current_dev_);
#endif
out:
    return (status);
...
int nvshmemi_nvls_rsc::unbind_group_mem(hipMemGenericAllocationHandle_t *mc_handle, off_t mc_offset,
                                        size_t mem_size) {
    int status = -1;
#if !__HIP__
    status = cuMulticastUnbind(*mc_handle, current_dev_, mc_offset, mem_size);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
...
int nvshmemi_nvls_rsc::map_group_mem(hipMemGenericAllocationHandle_t *mc_handle, size_t mem_size,
                                     off_t mem_offset, off_t mc_offset) {
    int status = -1;
#if !__HIP__
    hipMemAccessDesc access;
    status = hipMemMap((hipDeviceptr_t)NVSHMEMI_SYMMETRIC_HEAP_OFFSET(mc_base_ptr_, mc_offset),
                            mem_size, mem_offset, *mc_handle, 0);
    NVSHMEMI_NE_ERROR_JMP(
        status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
        "hipMemMap failed to map %ld bytes handle at address: %p offset %ld on device %d\n",
    ...
    status = hipMemSetAccess((hipDeviceptr_t)NVSHMEMI_SYMMETRIC_HEAP_OFFSET(mc_base_ptr_, mc_offset),
                             mem_size, (const hipMemAccessDesc *)&access, 1);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipMemSetAccess failed for address: %p on device %d\n",
...
int nvshmemi_nvls_rsc::unmap_group_mem(off_t mc_offset, uint64_t mem_size) {
    int status = -1;
#if !__HIP__
    status = hipMemUnmap((hipDeviceptr_t)NVSHMEMI_SYMMETRIC_HEAP_OFFSET(mc_base_ptr_, mc_offset), mem_size);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipMemUnmap failed to map %ld bytes at address: %p\n", mem_size,
