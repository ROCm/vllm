...
#include <hip/hip_runtime.h>                                                  // for hipMemcpy
...
#include "device_host/nvshmem_common.hip.h"                                  // for nvshmemi_pe_i...
...
static void nvshmemi_team_alloc_device(void) {
    CUDA_RUNTIME_CHECK(hipMalloc((void **)&nvshmemi_device_team_world, sizeof(nvshmemi_team_t)));
    CUDA_RUNTIME_CHECK(hipMalloc((void **)&nvshmemi_device_team_shared, sizeof(nvshmemi_team_t)));
    CUDA_RUNTIME_CHECK(hipMalloc((void **)&nvshmemi_device_team_node, sizeof(nvshmemi_team_t)));
    CUDA_RUNTIME_CHECK(
        hipMalloc((void **)&nvshmemi_device_team_same_mype_node, sizeof(nvshmemi_team_t)));
    CUDA_RUNTIME_CHECK(
        hipMalloc((void **)&nvshmemi_device_team_same_gpu, sizeof(nvshmemi_team_t)));
    CUDA_RUNTIME_CHECK(
        hipMalloc((void **)&nvshmemi_device_team_gpu_leaders, sizeof(nvshmemi_team_t)));
}

static void nvshmemi_team_update_device(void) {
    CUDA_RUNTIME_CHECK(hipMemcpy(nvshmemi_device_team_world, &nvshmemi_team_world,
                                  sizeof(nvshmemi_team_t), hipMemcpyHostToDevice));
    CUDA_RUNTIME_CHECK(hipMemcpy(&nvshmemi_device_team_pool[NVSHMEM_TEAM_WORLD_INDEX],
                                  &nvshmemi_device_team_world, sizeof(nvshmemi_team_t *),
                                  hipMemcpyHostToDevice));

    CUDA_RUNTIME_CHECK(hipMemcpy(nvshmemi_device_team_shared, &nvshmemi_team_shared,
                                  sizeof(nvshmemi_team_t), hipMemcpyHostToDevice));
    CUDA_RUNTIME_CHECK(hipMemcpy(&nvshmemi_device_team_pool[NVSHMEM_TEAM_SHARED_INDEX],
                                  &nvshmemi_device_team_shared, sizeof(nvshmemi_team_t *),
                                  hipMemcpyHostToDevice));

    CUDA_RUNTIME_CHECK(hipMemcpy(nvshmemi_device_team_node, &nvshmemi_team_node,
                                  sizeof(nvshmemi_team_t), hipMemcpyHostToDevice));
    CUDA_RUNTIME_CHECK(hipMemcpy(&nvshmemi_device_team_pool[NVSHMEM_TEAM_NODE_INDEX],
                                  &nvshmemi_device_team_node, sizeof(nvshmemi_team_t *),
                                  hipMemcpyHostToDevice));

    CUDA_RUNTIME_CHECK(hipMemcpy(nvshmemi_device_team_same_mype_node,
                                  &nvshmemi_team_same_mype_node, sizeof(nvshmemi_team_t),
                                  hipMemcpyHostToDevice));
    CUDA_RUNTIME_CHECK(hipMemcpy(&nvshmemi_device_team_pool[NVSHMEM_TEAM_SAME_MYPE_NODE_INDEX],
                                  &nvshmemi_device_team_same_mype_node, sizeof(nvshmemi_team_t *),
                                  hipMemcpyHostToDevice));

    CUDA_RUNTIME_CHECK(hipMemcpy(nvshmemi_device_team_same_gpu, &nvshmemi_team_same_gpu,
                                  sizeof(nvshmemi_team_t), hipMemcpyHostToDevice));
    CUDA_RUNTIME_CHECK(hipMemcpy(&nvshmemi_device_team_pool[NVSHMEM_TEAM_SAME_GPU_INDEX],
                                  &nvshmemi_device_team_same_gpu, sizeof(nvshmemi_team_t *),
                                  hipMemcpyHostToDevice));

    CUDA_RUNTIME_CHECK(hipMemcpy(nvshmemi_device_team_gpu_leaders, &nvshmemi_team_gpu_leaders,
                                  sizeof(nvshmemi_team_t), hipMemcpyHostToDevice));
    CUDA_RUNTIME_CHECK(hipMemcpy(&nvshmemi_device_team_pool[NVSHMEM_TEAM_GPU_LEADERS_INDEX],
...
static void nvshmemi_recexchalgo_get_neighbors(nvshmemi_team_t *teami) {
    ...
    CUDA_RUNTIME_CHECK(hipMalloc(&step1_recvfrom_device, sizeof(int) * (k - 1)));
    CUDA_RUNTIME_CHECK(hipMalloc(
        &step2_nbrs_device,
        sizeof(int *) * (step2_nphases + 1))); /* + 1 to make it non-zero otherwise hipMalloc
                                                  returns error when step2_nphases is 0 */

    for (int i = 0; i < step2_nphases; i++) {
        void *dev_ptr;
        CUDA_RUNTIME_CHECK(hipMalloc(&dev_ptr, sizeof(int) * (k - 1)));
        CUDA_RUNTIME_CHECK(hipMemcpy((int **)step2_nbrs_device + i, &dev_ptr, sizeof(int *),
                                      hipMemcpyHostToDevice));
    }
    CUDA_RUNTIME_CHECK(hipMemcpy(step1_recvfrom_device, step1_recvfrom, sizeof(int) * step1_nrecvs,
                                  hipMemcpyHostToDevice));
    void *dev_ptr, *dev_ptr_2;
    dev_ptr = step2_nbrs_device;
    for (int i = 0; i < step2_nphases; i++) {
        CUDA_RUNTIME_CHECK(
            hipMemcpy(&dev_ptr_2, (int **)dev_ptr + i, sizeof(int *), hipMemcpyDeviceToHost));
        CUDA_RUNTIME_CHECK(
            hipMemcpy(dev_ptr_2, step2_nbrs[i], sizeof(int) * (k - 1), hipMemcpyHostToDevice));
    }
...
static void nvshmemi_recexchalgo_free_mem(nvshmemi_team_t *teami) {
    CUDA_RUNTIME_CHECK(hipFree(teami->reduce_recexch.step1_recvfrom));
    for (int i = 0; i < teami->reduce_recexch.step2_nphases; i++) {
        void *dev_ptr;
        CUDA_RUNTIME_CHECK(hipMemcpy(&dev_ptr, teami->reduce_recexch.step2_nbrs + i, sizeof(int *),
                                      hipMemcpyDeviceToHost));
        CUDA_RUNTIME_CHECK(hipFree(dev_ptr));
    }
    CUDA_RUNTIME_CHECK(hipFree(teami->reduce_recexch.step2_nbrs));
}
...
void nvshmemi_team_init_nccl_comm(nvshmemi_team_t *teami) {
    ...
    if (teami->my_pe == 0) {
        ...
        CUDA_RUNTIME_CHECK(hipMemcpy(pWrk, &Id, sizeof(ncclUniqueId), hipMemcpyHostToDevice));
        CUDA_RUNTIME_CHECK(hipDeviceSynchronize());
        for (int i = 0; i < size; i++) {
            nvshmemx_char_put_nbi_on_stream((char *)pWrk, (const char *)pWrk, sizeof(ncclUniqueId),
                                            start + i * stride, (hipStream_t)0);
        }
        CUDA_RUNTIME_CHECK(hipDeviceSynchronize());
        nvshmemi_barrier(teami->team_idx);
    } else {
        nvshmemi_barrier(teami->team_idx);
        CUDA_RUNTIME_CHECK(hipMemcpy(&Id, pWrk, sizeof(ncclUniqueId), hipMemcpyDeviceToHost));
    }
...
static void nvshmemi_team_destroy_nvls(nvshmemi_team_t *team) {
    ...
    if (nvls_obj->get_refcount() == 0) { /* Last reference */
        ...
        delete nvls_obj;
        hipFree(team->nvls_rsc_base_ptr);
...
static int nvshmemi_team_create_nvls(nvshmemi_team_t *team) {
    ...
    CUDA_RUNTIME_CHECK(hipMalloc((void **)&team->nvls_rsc_base_ptr, sizeof(void *)));
    mc_heap_base = (uint64_t)(nvls_obj->get_mc_base());
    CUDA_RUNTIME_CHECK(
        hipMemcpy(team->nvls_rsc_base_ptr, &mc_heap_base, sizeof(void *), hipMemcpyHostToDevice));
...
team_shared_setup:
    ...
    CUDA_RUNTIME_CHECK(hipMalloc((void **)&nvshmemi_device_team_pool,
    ...
    if (nvshmemi_use_nccl) {
        ...
        if (nvshmemi_options.CUDA_LIMIT_STACK_SIZE_provided) {
            CUDA_RUNTIME_CHECK(
                hipDeviceSetLimit(hipLimitStackSize, nvshmemi_options.CUDA_LIMIT_STACK_SIZE));
            ...
            CUDA_RUNTIME_CHECK(hipDeviceSetLimit(hipLimitStackSize, 1256));
...
cleanup:
    ...
    if (status != NVSHMEMX_SUCCESS) {
        if (nvshmemi_team_pool) {
            ...
            hipFree(nvshmemi_device_team_pool);
...
int nvshmemi_team_finalize(void) {
    ...
    free(nvshmemi_team_pool);
    nvshmemi_team_pool = NULL;
    CUDA_RUNTIME_CHECK(hipFree(nvshmemi_device_team_pool));
    ...
    free(team_ret_val);
    nvshmemi_free(device_team_ret_val);
    hipFree(nvshmemi_device_team_world);
    hipFree(nvshmemi_device_team_shared);
    hipFree(nvshmemi_device_team_node);
    hipFree(nvshmemi_device_team_same_mype_node);
    hipFree(nvshmemi_device_team_same_gpu);
    hipFree(nvshmemi_device_team_gpu_leaders);
...
int nvshmemi_team_split_strided(nvshmemi_team_t *parent_team, int PE_start, int PE_stride,
    ...
    *team_ret_val_reduced = 0;
    ...
    if (my_pe >= 0) {
        ...
        CUDA_RUNTIME_CHECK(hipMemcpy(device_psync_pool_avail, psync_pool_avail, N_PSYNC_BYTES,
                                      hipMemcpyHostToDevice));
        CUDA_RUNTIME_CHECK(hipDeviceSynchronize());
        ...
        CUDA_RUNTIME_CHECK(hipMemcpy(psync_pool_avail_reduced, device_psync_pool_avail_reduced,
                                      N_PSYNC_BYTES, hipMemcpyDeviceToHost));
        ...
        if (myteam->team_idx == -1 || myteam->team_idx >= (int)nvshmemi_max_teams) {
            ...
            *new_team = myteam->team_idx;
            ...
            CUDA_RUNTIME_CHECK(hipMalloc((void **)&device_team_addr, sizeof(nvshmemi_team_t)));
            nvshmemi_team_set_p2p_connectivity(myteam);
            nvshmemi_recexchalgo_get_neighbors(myteam);
            CUDA_RUNTIME_CHECK(hipMemcpy(device_team_addr, myteam, sizeof(nvshmemi_team_t),
                                          hipMemcpyHostToDevice));
            CUDA_RUNTIME_CHECK(hipMemcpy(&nvshmemi_device_team_pool[myteam->team_idx],
            ...
            if (!myteam->is_team_node && !myteam->is_team_same_mype_node) {
                ...
                CUDA_RUNTIME_CHECK(hipMemcpy(device_psync_pool_avail, &team_npes_node, sizeof(int),
                                              hipMemcpyHostToDevice));
                CUDA_RUNTIME_CHECK(hipDeviceSynchronize());
                ...
                CUDA_RUNTIME_CHECK(hipMemcpy(&team_npes_node, device_psync_pool_avail_reduced,
    ...
    CUDA_RUNTIME_CHECK(
        hipMemcpy(device_team_ret_val, team_ret_val, sizeof(int), hipMemcpyHostToDevice));
    CUDA_RUNTIME_CHECK(hipDeviceSynchronize());
    ...
    CUDA_RUNTIME_CHECK(hipMemcpy(team_ret_val_reduced, device_team_ret_val_reduced, sizeof(int),
                                  hipMemcpyDeviceToHost));
...
void nvshmemi_team_destroy(nvshmemi_team_t *team) {
    ...
    CUDA_RUNTIME_CHECK(hipMemset(&nvshmemi_device_team_pool[idx], 0, sizeof(nvshmemi_team_t *)));
    ...
    nvshmemi_team_t *device_team_addr;
    CUDA_RUNTIME_CHECK(hipMemcpy((void **)&device_team_addr, &nvshmemi_device_team_pool[idx],
                                  sizeof(nvshmemi_team_t *), hipMemcpyDeviceToHost));
    CUDA_RUNTIME_CHECK(hipFree(device_team_addr));
}
