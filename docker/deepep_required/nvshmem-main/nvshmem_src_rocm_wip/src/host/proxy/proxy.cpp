...
#include <hip/hip_runtime.h>                          // for hipSuccess
#include <hip/hip_runtime.h>                  // for hipHostFree
...
#include "device_host/nvshmem_common.hip.h"  // for nvshmemi_devi...
...
int nvshmemi_proxy_prep_minimal_state(proxy_state_t *state) {
    ...
    CUDA_RUNTIME_CHECK(hipHostGetDevicePointer(reinterpret_cast<void**>(&temp_global_exit_request_state),
                                                reinterpret_cast<void*>(state->global_exit_request_state), 0));
    CUDA_RUNTIME_CHECK(
        hipHostGetDevicePointer(reinterpret_cast<void**>(&temp_global_exit_code), reinterpret_cast<void*>(state->global_exit_code), 0));
    CUDA_RUNTIME_CHECK(
        hipHostGetDevicePointer(reinterpret_cast<void**>(&nvshmemi_timeout_dptr), reinterpret_cast<void*>(state->nvshmemi_timeout), 0));
...
int nvshmemi_proxy_setup_device_channels(proxy_state_t *state) {
    ...
    CUDA_RUNTIME_CHECK(
        hipMalloc(&state->channels_device, sizeof(proxy_channel_t) * state->channel_count));
    ...
    CUDA_RUNTIME_CHECK(hipHostGetDevicePointer(reinterpret_cast<void**>(&temp_buf_dptr), reinterpret_cast<void*>(state->channels[0].buf), 0));
    CUDA_RUNTIME_CHECK(
        hipHostGetDevicePointer(reinterpret_cast<void**>(&temp_complete_dptr), reinterpret_cast<void*>(state->channels[0].complete), 0));
    CUDA_RUNTIME_CHECK(
        hipHostGetDevicePointer(reinterpret_cast<void**>(&temp_quiet_issue_dptr), reinterpret_cast<void*>(state->channels[0].quiet_issue), 0));
    CUDA_RUNTIME_CHECK(
        hipHostGetDevicePointer(reinterpret_cast<void**>(&temp_quiet_ack_dptr), reinterpret_cast<void*>(state->channels[0].quiet_ack), 0));
    CUDA_RUNTIME_CHECK(
        hipHostGetDevicePointer(reinterpret_cast<void**>(&temp_cst_issue_dptr), reinterpret_cast<void*>(state->channels[0].cst_issue), 0));
    CUDA_RUNTIME_CHECK(hipHostGetDevicePointer(reinterpret_cast<void**>(&temp_cst_ack_dptr), reinterpret_cast<void*>(state->channels[0].cst_ack), 0));
    ...
    CUDA_RUNTIME_CHECK(hipMalloc((void **)&proxy_channel_g_buf_head_ptr, sizeof(uint64_t)));
    CUDA_RUNTIME_CHECK(hipMemset((void *)proxy_channel_g_buf_head_ptr, 0, sizeof(uint64_t)));

    uint64_t *proxy_channels_complete_local_ptr;
    CUDA_RUNTIME_CHECK(hipMalloc((void **)&proxy_channels_complete_local_ptr, sizeof(uint64_t)));
    CUDA_RUNTIME_CHECK(hipMemset((void *)proxy_channels_complete_local_ptr, 0, sizeof(uint64_t)));
...
int nvshmemi_proxy_create_channels(proxy_state_t *proxy_state) {
    ...
    for (int i = 0; i < proxy_state->channel_count; i++) {
        // for put/get
        CUDA_RUNTIME_CHECK(hipHostMalloc((void **)&channels[i].buf, proxy_state->channel_bufsize,
                                          0)); /* CPU reads, GPU writes */
        memset(channels[i].buf, 0, proxy_state->channel_bufsize);

        CUDA_RUNTIME_CHECK(hipMalloc(
            &channels[i].issue, sizeof(uint64_t))); /* issue is not accessed through LD/ST by CPU
                                                       thread, therefore on device memory */
        CUDA_RUNTIME_CHECK(hipMemset(channels[i].issue, 0, sizeof(uint64_t)));

        CUDA_RUNTIME_CHECK(hipHostMalloc((void **)&channels[i].complete, sizeof(uint64_t),
                                          0)); /* CPU writes, GPU reads */
        CUDA_RUNTIME_CHECK(hipHostMalloc((void **)&channels[i].quiet_issue, sizeof(uint64_t),
                                          0)); /* CPU reads, GPU writes */
        CUDA_RUNTIME_CHECK(hipHostMalloc((void **)&channels[i].quiet_ack, sizeof(uint64_t),
                                          0)); /* CPU writes, GPU reads */
        CUDA_RUNTIME_CHECK(hipHostMalloc((void **)&channels[i].cst_issue, sizeof(uint64_t),
                                          0)); /* CPU reads, GPU writes */
        CUDA_RUNTIME_CHECK(hipHostMalloc((void **)&channels[i].cst_ack, sizeof(uint64_t),
                                          0)); /* CPU writes, GPU reads */
...
int nvshmemi_proxy_init(nvshmemi_state_t *state, int proxy_level) {
    ...
    CUDA_RUNTIME_CHECK(
        hipHostMalloc((void **)&proxy_state->global_exit_request_state, sizeof(int), 0));
    CUDA_RUNTIME_CHECK(hipHostMalloc((void **)&proxy_state->global_exit_code, sizeof(int), 0));
    CUDA_RUNTIME_CHECK(hipHostMalloc((void **)&proxy_state->nvshmemi_timeout,
    ...
    CUDA_RUNTIME_CHECK(hipStreamCreateWithFlags(&proxy_state->stream, hipStreamNonBlocking));
    CUDA_RUNTIME_CHECK(
        hipStreamCreateWithFlags(&proxy_state->queue_stream_out, hipStreamNonBlocking));
    CUDA_RUNTIME_CHECK(
        hipStreamCreateWithFlags(&proxy_state->queue_stream_in, hipStreamNonBlocking));
    CUDA_RUNTIME_CHECK(hipEventCreateWithFlags(&proxy_state->cuev, hipEventDefault));
    ...
    status = hipDeviceGetAttribute(
        &write_options,
        (hipDeviceAttribute_t)CU_DEVICE_ATTRIBUTE_GPU_DIRECT_RDMA_FLUSH_WRITES_OPTIONS, device);
    if (status != hipSuccess) {
        proxy_state->is_consistency_api_supported = false;
        hipGetLastError();
        goto post_cst_api_check;
    }
    if (write_options & (hipDeviceAttribute_t)hipFlushGPUDirectRDMAWritesOptionHost)
        proxy_state->is_consistency_api_supported = true;
    status = hipDeviceGetAttribute(
        &proxy_state->gdr_device_native_ordering,
        (hipDeviceAttribute_t)CU_DEVICE_ATTRIBUTE_GPU_DIRECT_RDMA_WRITES_ORDERING, device);
    if (status != hipSuccess) {
        proxy_state->gdr_device_native_ordering = 0;
        hipGetLastError();
    }
...
void enforce_cst(proxy_state_t *proxy_state) {
    ...
    if (proxy_state->is_consistency_api_supported) {
        if (CU_FLUSH_GPU_DIRECT_RDMA_WRITES_TO_OWNER > proxy_state->gdr_device_native_ordering &&
            ...
            status = cuFlushGPUDirectRDMAWrites(CU_FLUSH_GPU_DIRECT_RDMA_WRITES_TARGET_CURRENT_CTX,
    ...
    status = hipEventRecord(proxy_state->cuev, proxy_state->stream);
    if (unlikely(status != hipSuccess)) {
        NVSHMEMI_ERROR_EXIT("hipEventRecord() failed in the proxy thread \n");
    }
#elif defined(NVSHMEM_X86_64)
    for (int i = 0; i < state->num_initialized_transports; i++) {
...
void *nvshmemi_proxy_progress(void *in) {
    ...
    curesult = hipCtxSetCurrent(proxy_state->nvshmemi_state->cucontext);
...
int nvshmemi_proxy_finalize(nvshmemi_state_t *state) {
    ...
    if (proxy_state->stream) CUDA_RUNTIME_CHECK(hipStreamDestroy(proxy_state->stream));
    if (proxy_state->queue_stream_in)
        CUDA_RUNTIME_CHECK(hipStreamDestroy(proxy_state->queue_stream_in));
    if (proxy_state->queue_stream_out)
        CUDA_RUNTIME_CHECK(hipStreamDestroy(proxy_state->queue_stream_out));
    ...
    if (nvshmemi_device_state.proxy_channels_complete_local_ptr)
        CUDA_RUNTIME_CHECK(hipFree(nvshmemi_device_state.proxy_channels_complete_local_ptr));
    if (nvshmemi_device_state.proxy_channel_g_buf_head_ptr)
        CUDA_RUNTIME_CHECK(hipFree(nvshmemi_device_state.proxy_channel_g_buf_head_ptr));
    if (proxy_state->channels_device) CUDA_RUNTIME_CHECK(hipFree(proxy_state->channels_device));

    /* create channels state */
    for (int i = 0; i < proxy_state->channel_count; i++) {
        CUDA_RUNTIME_CHECK(hipHostFree(proxy_state->channels[i].complete));
        CUDA_RUNTIME_CHECK(hipHostFree(proxy_state->channels[i].quiet_issue));
        CUDA_RUNTIME_CHECK(hipHostFree(proxy_state->channels[i].quiet_ack));
        CUDA_RUNTIME_CHECK(hipHostFree(proxy_state->channels[i].cst_issue));
        CUDA_RUNTIME_CHECK(hipHostFree(proxy_state->channels[i].cst_ack));
        CUDA_RUNTIME_CHECK(hipFree(proxy_state->channels[i].issue));
        CUDA_RUNTIME_CHECK(hipHostFree(proxy_state->channels[i].buf));
    }
    ...
    if (proxy_state->global_exit_request_state)
        CUDA_RUNTIME_CHECK(hipHostFree(proxy_state->global_exit_request_state));
    if (proxy_state->global_exit_code)
        CUDA_RUNTIME_CHECK(hipHostFree(proxy_state->global_exit_code));
    if (proxy_state->nvshmemi_timeout)
        CUDA_RUNTIME_CHECK(hipHostFree(proxy_state->nvshmemi_timeout));
