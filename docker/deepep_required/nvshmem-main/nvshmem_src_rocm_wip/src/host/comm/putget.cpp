...
#include <hip/hip_runtime.h>                                    // for hipMemcpyAsync
...
#define NOT_A_CUDA_STREAM ((hipStream_t)0)
...
static int nvshmemi_p2p_rma_optimized(hipStream_t custrm /* internal stream */, hipEvent_t cuev,
    ...
    if (verb.is_stream) {
        if (verb.is_nbi) {
            CUDA_RUNTIME_CHECK_GOTO(hipEventRecord(cuev, verb.cstrm), status, out);
            CUDA_RUNTIME_CHECK_GOTO(hipStreamWaitEvent(custrm, cuev, 0), status, out);
            if (is_contig) { /*can include iput,iget in future*/
                CUDA_RUNTIME_CHECK_GOTO(
                    hipMemcpyAsync(dest->ptr, src->ptr, bytesdesc.nelems * bytesdesc.elembytes,
                                    hipMemcpyDeviceToDevice, custrm),
                    status, out);
            ...
            if (is_contig) {
                if (is_single_word) {
                    if (verb.desc == NVSHMEMI_OP_P) {
                        CUDA_RUNTIME_CHECK_GOTO(
                            hipMemcpyAsync(dest->ptr, src->ptr,
                                            bytesdesc.nelems * bytesdesc.elembytes,
                                            hipMemcpyHostToDevice, verb.cstrm),
                            status, out);
                    } else { /*!is P*/
                        CUDA_RUNTIME_CHECK_GOTO(
                            hipMemcpyAsync(dest->ptr, src->ptr,
                                            bytesdesc.nelems * bytesdesc.elembytes,
                                            hipMemcpyDeviceToHost, verb.cstrm),
                            status, out);
                    }
                } else { /*!is_single_word*/
                    CUDA_RUNTIME_CHECK_GOTO(
                        hipMemcpyAsync(dest->ptr, src->ptr, bytesdesc.nelems * bytesdesc.elembytes,
                                        hipMemcpyDeviceToDevice, verb.cstrm),
                        status, out);
                    if (verb.desc == NVSHMEMI_OP_PUT_SIGNAL)
                ...
                CUDA_RUNTIME_CHECK_GOTO(
                    hipMemcpy2DAsync(dest->ptr, bytesdesc.deststride * bytesdesc.elembytes,
                ...
                if (is_single_word) {
                    if (verb.desc == NVSHMEMI_OP_P) {
                        CUDA_RUNTIME_CHECK_GOTO(
                            hipMemcpyAsync(dest->ptr, src->ptr,
                                            bytesdesc.nelems * bytesdesc.elembytes,
                                            hipMemcpyHostToDevice, custrm),
                            status, out);
                    } else { /*!is P*/
                        CUDA_RUNTIME_CHECK_GOTO(
                            hipMemcpyAsync(dest->ptr, src->ptr,
                                            bytesdesc.nelems * bytesdesc.elembytes,
                                            hipMemcpyDeviceToHost, custrm),
                            status, out);
                    }
                } else { /*!is_single_word*/
                    CUDA_RUNTIME_CHECK_GOTO(
...
static int nvshmemi_p2p_rma_registered(hipStream_t custrm /* internal stream */, hipEvent_t cuev,
                                       rma_verb_t verb, rma_memdesc_t *dest, rma_memdesc_t *src,
                                       rma_bytesdesc_t bytesdesc, uint64_t *sig_addr,
                                       uint64_t signal, int sig_op, int pe) {
    hipStream_t stream_for_op;
    int status = 0;

    if ((!verb.is_stream) || verb.is_nbi) {
        stream_for_op = (hipStream_t)custrm;
    } else {
    ...
    if ((bytesdesc.srcstride == 1) && (bytesdesc.deststride == 1)) {
        status = hipMemcpyAsync(dest->ptr, src->ptr, bytesdesc.nelems * bytesdesc.elembytes,
                                 hipMemcpyDefault, stream_for_op);
        ...
        status = hipMemcpy2DAsync(dest->ptr, bytesdesc.deststride * bytesdesc.elembytes, src->ptr,
                                   bytesdesc.srcstride * bytesdesc.elembytes, bytesdesc.elembytes,
                                   bytesdesc.nelems, hipMemcpyDefault, stream_for_op);
        NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "hipMemcpy2DAsync() failed\n");
    }
...
static inline int nvshmemi_prepare_and_post_mapped_rma(rma_verb_t verb, size_t nelems,
                                                       ...
                                                       int sig_op, int pe) {
    hipStream_t custrm = nvshmemi_state->custreams[pe % MAX_PEER_STREAMS];
    hipEvent_t cuev = nvshmemi_state->cuevents[pe % MAX_PEER_STREAMS];
...
#define NVSHMEM_TYPE_PUT(Name, TYPE)                                                              \
    NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_##Name##_put(TYPE *dest, const TYPE *source, size_t nelems, int pe) {            \
...
#define NVSHMEM_PUTSIZE(Name, Type)                                                              \
    NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_put##Name(void *dest, const void *source, size_t nelems, int pe) {              \
        NVSHMEMI_CHECK_INIT_STATUS();                                                            \
...
NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_putmem(void *dest, const void *source, size_t bytes, int pe) {
    NVSHMEMI_CHECK_INIT_STATUS();
    INFO(NVSHMEM_P2P,
...
#define NVSHMEMX_TYPE_P_ON_STREAM(Name, TYPE)                                                      \
    void nvshmemx_##Name##_p_on_stream(TYPE *dest, const TYPE value, int pe, hipStream_t cstrm) { \
        NVSHMEMI_CHECK_INIT_STATUS();                                                              \
...
#define NVSHMEM_TYPE_IPUT(Name, TYPE)                                                              \
    NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_##Name##_iput(TYPE *dest, const TYPE *source, ptrdiff_t dst, ptrdiff_t sst,       \
...
#define NVSHMEM_IPUTSIZE(Name, Type)                                                              \
    NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_iput##Name(void *dest, const void *source, ptrdiff_t dst, ptrdiff_t sst,         \
                            size_t nelems, int pe) {                                              \
        NVSHMEMI_CHECK_INIT_STATUS();                                                             \
...
#define NVSHMEM_TYPE_PUT_NBI(type, TYPE)                                                           \
    NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_##type##_put_nbi(TYPE *dest, const TYPE *source, size_t nelems, int pe) {         \
        NVSHMEMI_CHECK_INIT_STATUS();                                                              \
...
#define NVSHMEM_PUTSIZE_NBI(Name, Type)                                                           \
    NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_put##Name##_nbi(void *dest, const void *source, size_t nelems, int pe) {         \
        NVSHMEMI_CHECK_INIT_STATUS();                                                             \
...
NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_putmem_nbi(void *dest, const void *source, size_t bytes, int pe) {
#if !__HIP__
...
#define NVSHMEMX_TYPE_PUT_SIGNAL_NBI_ON_STREAM(type, TYPE)                                         \
    void nvshmemx_##type##_put_signal_nbi_on_stream(TYPE *dest, const TYPE *source, size_t nelems, \
                                                    ...
                                                    int sig_op, int pe, hipStream_t cstrm) {      \
        NVSHMEMI_CHECK_INIT_STATUS();                                                              \
...
#define NVSHMEM_TYPE_GET(Name, TYPE)                                                              \
    NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_##Name##_get(TYPE *dest, const TYPE *source, size_t nelems, int pe) {            \
        NVSHMEMI_CHECK_INIT_STATUS();                                                             \
...
#define NVSHMEM_GETSIZE(Name, Type)                                                              \
    NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_get##Name(void *dest, const void *source, size_t nelems, int pe) {              \
        NVSHMEMI_CHECK_INIT_STATUS();                                                            \
...
NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_getmem(void *dest, const void *source, size_t bytes, int pe) {
#if !__HIP__
    NVTX_FUNC_RANGE_IN_GROUP(RMA_BLOCKING);
#endif
    NVSHMEMI_CHECK_INIT_STATUS();
...
#define NVSHMEM_TYPE_G(Name, TYPE)                                                            \
    NVSHMEMI_HOSTDEVICE_PREFIX TYPE nvshmem_##Name##_g(const TYPE *source, int pe) {                                     \
        NVSHMEMI_CHECK_INIT_STATUS();                                                         \
        TYPE value;                                                                           \
...
#define NVSHMEMX_TYPE_G_ON_STREAM(Name, TYPE)                                                      \
    TYPE nvshmemx_##Name##_g_on_stream(const TYPE *source, int pe, hipStream_t cstrm) {           \
        NVSHMEMI_CHECK_INIT_STATUS();                                                              \
        TYPE value;                                                                                \
        INFO(NVSHMEM_P2P, "[%d] single get : (remote)source %p, remote PE %d",                     \
             nvshmemi_state->mype, source, pe);                                                    \
...
#define NVSHMEM_TYPE_IGET(Name, TYPE)                                                              \
    NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_##Name##_iget(TYPE *dest, const TYPE *source, ptrdiff_t dst, ptrdiff_t sst,       \
                               size_t nelems, int pe) {                                            \
        NVSHMEMI_CHECK_INIT_STATUS();                                                              \
...
#define NVSHMEM_IGETSIZE(Name, Type)                                                              \
    NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_iget##Name(void *dest, const void *source, ptrdiff_t dst, ptrdiff_t sst,         \
                            size_t nelems, int pe) {                                              \
        NVSHMEMI_CHECK_INIT_STATUS();                                                             \
...
#define NVSHMEM_TYPE_GET_NBI(type, TYPE)                                                           \
    NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_##type##_get_nbi(TYPE *dest, const TYPE *source, size_t nelems, int pe) {         \
        NVSHMEMI_CHECK_INIT_STATUS();                                                              \
...
#define NVSHMEM_GETSIZE_NBI(Name, Type)                                                           \
    NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_get##Name##_nbi(void *dest, const void *source, size_t nelems, int pe) {         \
        NVSHMEMI_CHECK_INIT_STATUS();                                                             \
...
NVSHMEMI_HOSTDEVICE_PREFIX void nvshmem_getmem_nbi(void *dest, const void *source, size_t bytes, int pe) {
#if !__HIP__
    NVTX_FUNC_RANGE_IN_GROUP(RMA_NONBLOCKING);
#endif
    NVSHMEMI_CHECK_INIT_STATUS();
