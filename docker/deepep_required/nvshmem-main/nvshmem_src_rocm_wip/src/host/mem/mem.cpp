...
#include <hip/hip_runtime.h>                                        // for cudaHostUn...
...
#include "device_host/nvshmem_common.hip.h"                        // for nvshmemi_d...
...
static int buffer_register(nvshmem_transport_t transport, void *addr, size_t length) {
    ...
    status = hipPointerGetAttributes(&attr, addr);
    ...
    if (attr.type == hipMemoryTypeManaged) {
    ...
    if (attr.type == hipMemoryTypeUnregistered) {
        ...
        status = hipHostRegister(addr, length, hipHostRegisterDefault);
        if (status) {
...
out_error_unlocked:
    if (handle) {
        if (handle->registered_by_us) {
            CUDA_RUNTIME_CHECK(hipHostUnregister(addr));
        }
        ...
        if (addr > tmp_handle->ptr) {
            ...
            do {
                ...
                if (tmp_handle->registered_by_us && !tmp_handle->linked_with_prev) {
                    CUDA_RUNTIME_CHECK(hipHostUnregister(tmp_handle->ptr));
                }
                num_mem_handles++;
...
static void buffer_unregister_all(nvshmem_transport_t transport) {
    ...
    for (size_t i = 0; i < cache->array_used; i++) {
        ...
        if (cache->buffers[i]->registered_by_us && !cache->buffers[i]->linked_with_prev) {
            CUDA_RUNTIME_CHECK(hipHostUnregister(cache->buffers[i]->ptr));
        }
