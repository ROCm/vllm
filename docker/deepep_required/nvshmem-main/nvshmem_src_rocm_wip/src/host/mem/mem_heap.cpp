...
#include <hip/hip_runtime.h>                                                          // for CUDA_...
#include <hip/hip_runtime.h>                                                  // for hipFree
...
#include "device_host/nvshmem_common.hip.h"                                  // for nvshm...
...
static_assert(sizeof(hipMemGenericAllocationHandle_t) <= NVSHMEM_MEM_HANDLE_SIZE,
              "sizeof(hipMemGenericAllocationHandle_t) <= NVSHMEM_MEM_HANDLE_SIZE");
...
int nvshmemi_symmetric_heap_vidmem_static_pinned::allocate_heap_memory() {
    int status = 0;
    status = hipMalloc(&heap_base_, heap_size_);
...
int nvshmemi_symmetric_heap_sysmem_static_shm::allocate_heap_memory() {
    ...
    status = nvshmemi_boot_handle.barrier(&nvshmemi_boot_handle);
    CUDA_RUNTIME_CHECK(hipHostRegister(heap_info_.addr, shm_size, hipHostRegisterDefault));
    CUDA_RUNTIME_CHECK(hipHostGetDevicePointer(&global_heap_base_, heap_info_.addr, 0));
...
int nvshmemi_symmetric_heap_static::reserve_heap(void) {
    ...
    status = hipPointerSetAttribute(&data, HIP_POINTER_ATTRIBUTE_SYNC_MEMOPS, (hipDeviceptr_t)(heap_base_));
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
...
int nvshmemi_symmetric_heap_vidmem_dynamic_vmm::reserve_heap() {
    ...
    status = hipMemGetAllocationGranularity(&mem_granularity_, &prop,
                                                 hipMemAllocationGranularityRecommended);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipMemGetAllocationGranularity failed \n");
    ...
    physical_heap_size_ = 0;
    status = hipMemAddressReserve((hipDeviceptr_t *)&global_heap_base_, p2p_npes * heap_size_,
                                       alignbytes, (hipDeviceptr_t)NULL, 0);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
...
int nvshmemi_symmetric_heap_vidmem_dynamic_vmm::cleanup_symmetric_heap() {
    ...
    NVSHMEMU_FOR_EACH_IF(
        ...
        {NVSHMEMU_FOR_EACH_IF(
            j, handles_.size(), ((j == 0) || (j > 0 && !nvshmemi_device_state.enable_rail_opt)), {
                ...
                status = hipMemUnmap((hipDeviceptr_t)heap_base_, physical_heap_size_);
                NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                                      "release memory failed for p2p on heap dynamic (my PE)\n");
                NVSHMEMU_FOR_EACH(i, cumem_handles_.size()) {
                    status = hipMemRelease(std::get<0>(cumem_handles_[i]));
                    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                                          "hipMemRelease failed \n");
                }
    ...
    status = hipMemAddressFree((hipDeviceptr_t)global_heap_base_, reserved_heap_size_);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipMemAddressFree failed \n");
...
int nvshmemi_symmetric_heap_vidmem_static_pinned::export_memory(nvshmem_mem_handle_t *mem_handle,
    ...
    int status = 0;
    hipIpcMemHandle_t *ipc_handle = (hipIpcMemHandle_t *)mem_handle;

    assert(sizeof(hipIpcMemHandle_t) <= NVSHMEM_MEM_HANDLE_SIZE);
    INFO(NVSHMEM_MEM, "calling hipIpcGetMemHandle on buf: %p size: %zu", buf, length);

    status = hipIpcGetMemHandle(ipc_handle, buf);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INVALID_VALUE, out,
...
int nvshmemi_symmetric_heap_vidmem_dynamic_vmm::export_memory(nvshmem_mem_handle_t *mem_handle,
    ...
    int status = 0;
    hipMemGenericAllocationHandle_t *handle_in =
        reinterpret_cast<hipMemGenericAllocationHandle_t *>(mem_handle_in);
    INFO(NVSHMEM_MEM, "calling hipMemExportToShareableHandle on handle: %p", handle_in);
    status = hipMemExportToShareableHandle((void *)mem_handle, *handle_in,
                                                                    get_mem_handle_type(), 0);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipMemExportToShareableHandle failed \n");
out:
    return (status);
}
...
int nvshmemi_symmetric_heap_vidmem_dynamic_vmm::import_memory(nvshmem_mem_handle_t *mem_handle,
                                                              void **buf, size_t size) {
    int status = 0;
    hipMemGenericAllocationHandle_t peer_handle;
    ...
    if (get_mem_handle_type() == hipMemHandleTypePosixFileDescriptor) {
        int fd = *(int *)mem_handle;
        status = 
                       hipMemImportFromShareableHandle(&peer_handle, (void *)(uintptr_t)fd,
                                                      get_mem_handle_type());
    } else {
        status =
    ...
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipMemImportFromShareableHandle failed state->device_id : %d \n",
                          gpu_device_id);

    status = hipMemMap((hipDeviceptr_t)*buf, size, 0, peer_handle, 0);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipMemMap failed to map %ld bytes handle at address: %p\n", size, *buf);
    access.location.type = hipMemLocationTypeDevice;
    access.location.id = gpu_device_id;
    access.flags = hipMemAccessFlagsProtReadWrite;
    status = hipMemRelease(peer_handle);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipMemRelease failed \n");
    status = hipMemSetAccess((hipDeviceptr_t)*buf, size, (const hipMemAccessDesc *)&access, 1);
out:
    return (status);
}
...
int nvshmemi_symmetric_heap_vidmem_static_pinned::import_memory(nvshmem_mem_handle_t *mem_handle,
    ...
    status = hipIpcOpenMemHandle(buf, *ipc_handle, hipIpcMemLazyEnablePeerAccess);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INVALID_VALUE, out,
                          "hipIpcOpenMemHandle failed with error %d \n", status);
out:
    return (status);
}
...
int nvshmemi_symmetric_heap_vidmem_dynamic_vmm::release_memory(void *buf, size_t size) {
    int status = 0;
    status = hipMemUnmap((hipDeviceptr_t)buf, size);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INVALID_VALUE, out,
                          "hipMemUnmap failed with error %d \n", status);
out:
    return (status);
}
...
int nvshmemi_symmetric_heap_vidmem_static_pinned::release_memory(void *buf, size_t size) {
    int status = 0;
    status = hipIpcCloseMemHandle(buf);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INVALID_VALUE, out,
                          "hipIpcCloseMemHandle failed with error %d \n", status);
out:
    return (status);
}
...
int nvshmemi_symmetric_heap_vidmem_dynamic_vmm::nvls_broadcast_heap_handle_fabric(
    ...
    if (team->my_pe == root) {
        CUDA_RUNTIME_CHECK(hipMemcpy(pWrk, buffer, length, hipMemcpyHostToDevice));
        CUDA_RUNTIME_CHECK(hipDeviceSynchronize());
        for (int i = 0; i < team->size; i++) {
            nvshmemx_char_put_nbi_on_stream((char *)pWrk, (const char *)pWrk, length,
                                            team->start + i * team->stride, (hipStream_t)0);
        }
        CUDA_RUNTIME_CHECK(hipDeviceSynchronize());
        nvshmem_barrier(team->team_idx);
    } else {
        nvshmem_barrier(team->team_idx);
        CUDA_RUNTIME_CHECK(hipMemcpy(buffer, pWrk, length, hipMemcpyDeviceToHost));
        CUDA_RUNTIME_CHECK(hipDeviceSynchronize());
    }
    return (0);
}
...
int nvshmemi_symmetric_heap_vidmem_dynamic_vmm::nvls_create_heap_memory_by_size(
    ...
    hipMemGenericAllocationHandle_t *my_handle;
    hipMemGenericAllocationHandle_t peer_handle;
...
int nvshmemi_symmetric_heap_vidmem_dynamic_vmm::nvls_bind_heap_memory_by_size(
    ...
    size_t mmap_size) {
    int status = -1;
    hipMemGenericAllocationHandle_t *mc_handle = NULL;
    nvls::nvshmemi_nvls_rsc *nvls_obj = reinterpret_cast<nvls::nvshmemi_nvls_rsc *>(team->nvls_rsc);
    // Prune for duplicate teams that inherit the rsc, but own the resource
    if (!nvls_obj->is_owner(team)) return 0;
...
int nvshmemi_symmetric_heap_vidmem_dynamic_vmm::allocate_physical_memory_to_heap(size_t size) {
    ...
    do {
        ...
        status = hipMemCreate(&cumem_handle, register_size,
                                                       (const hipMemAllocationProp *)&prop, 0);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                              "hipMemCreate failed \n");
        ...
        status = hipMemMap((hipDeviceptr_t)buf_end, register_size, mmap_offset, cumem_handle, 0);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                              "hipMemMap failed \n");

        status = hipMemSetAccess((hipDeviceptr_t)buf_end, register_size,
                                                          (const hipMemAccessDesc *)&access, 1);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
