...
#include <hip/hip_runtime.h>                                                          // for hipDevice_t
#include <hip/hip_runtime.h>                                                  // for cudaG...
...
void nvshmemi_mem_p2p_transport::print_mem_handle(int pe_id, int transport_idx,
    ...
    INFO(NVSHMEM_INIT, "[%d] hipIpcOpenMemHandle fromhandle 0x%s", state->mype, hex);
    NVSHMEMU_HOST_PTR_FREE(hex);
    INFO(NVSHMEM_INIT, "[%d] hipIpcOpenMemHandle tobuf %p", state->mype,
...
nvshmemi_mem_p2p_transport::nvshmemi_mem_p2p_transport(int mype, int npes) {
    ...
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipCtxGetDevice failed \n");

    cudev = (hipDevice_t *)std::malloc(sizeof(hipDevice_t) * ndev);
    ...
    if (nvshmemi_cuda_driver_version >= 12040 && prop.major >= 9 &&
        ...
        nvshmemi_has_mnnvl_fabric_ = 1;
              hipDeviceGetAttribute(
                  &flag,
                  static_cast<hipDeviceAttribute_t>(CU_DEVICE_ATTRIBUTE_HANDLE_TYPE_FABRIC_SUPPORTED),
                  device_id);
        ...
        nvshmemi_mem_handle_type_ =
            (nvshmemi_has_mnnvl_fabric_ && flag)
                ? static_cast<hipMemAllocationHandleType>(CU_MEM_HANDLE_TYPE_FABRIC)
    ...
    if (nvshmemi_options.CUMEM_HANDLE_TYPE_provided) {
        if (strcmp_case_insensitive(nvshmemi_options.CUMEM_HANDLE_TYPE, "FABRIC") == 0)
            ...
            nvshmemi_mem_handle_type_ = hipMemHandleTypePosixFileDescriptor;
    }
