...
#include <hip/hip_runtime.h>                 // for hipMemcpy, hipMemset
...
void *mspace::allocate_zeroed(size_t n_elements, size_t elem_size) {
    ...
    if (ptr) CUDA_RUNTIME_CHECK(hipMemset(ptr, 0, bytes));
    ASSERT_CORRECTNESS
    return ptr;
}
...
void *mspace::reallocate(void *ptr, size_t size) {
    ...
    if (size < current_size) {
        ...
        CUDA_RUNTIME_CHECK(hipMemcpy(new_ptr, ptr, size, hipMemcpyDeviceToDevice));
        inuse_chunks.erase(ptr);
