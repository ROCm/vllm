...
mchunkptr mem2chunk(void* mem) {
    ...
    p_d = (hipDeviceptr_t)(((char*)mem) - sizeof(uintptr_t));
    CUDA_RUNTIME_CHECK(hipMemcpyAsync((void*)&p, p_d, sizeof(uintptr_t), hipMemcpyDeviceToHost,
...
#define nvshmem_adjust_mchunk(p, adjust_bytes)                                                    \
    do {                                                                                          \
        hipDeviceptr_t ptr = (hipDeviceptr_t)(p->nvshmem_info.ptr +                                     \
        ...
        CUDA_RUNTIME_CHECK(hipMemcpyAsync(ptr + adjust_bytes, ptr, sizeof(uintptr_t),            \
                                           hipMemcpyDeviceToDevice, nvshmemi_state->my_stream)); \
        CUDA_RUNTIME_CHECK(hipStreamSynchronize(nvshmemi_state->my_stream));                     \
...
#define nvshmem_suballocate_mchunk(p, psize, r)                                                    \
    do {                                                                                           \
        ...
        CUDA_RUNTIME_CHECK(hipMemcpyAsync(                                                        \
            p->nvshmem_info.ptr + align_offset(p->nvshmem_info.ptr + sizeof(uintptr_t)),           \
            (void*)&p, sizeof(uintptr_t), hipMemcpyHostToDevice, nvshmemi_state->my_stream));     \
        CUDA_RUNTIME_CHECK(hipStreamSynchronize(nvshmemi_state->my_stream));                      \
    } while (0)
#endif
...
static void init_top_nvshmem(mstate m, mchunkptr p, char* d, size_t dsize) {
    ...
    d = d + offset;
    CUDA_RUNTIME_CHECK(hipMemcpyAsync((hipDeviceptr_t)d, (void*)&p, sizeof(uintptr_t),
                                       hipMemcpyHostToDevice, nvshmemi_state->my_stream));
    CUDA_RUNTIME_CHECK(hipStreamSynchronize(nvshmemi_state->my_stream));
...
#if NVSHMEM_SINGLE_HEAP
    if (mem != 0 && calloc_must_clear(mem2chunk(mem))) hipMemset(mem, 0, req);
#else
    if (mem != 0 && calloc_must_clear(mem2chunk(mem))) memset(mem, 0, req);
#endif
    return mem;
