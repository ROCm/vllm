...
int nvshmemi_update_device_state() {
    ...
    if (!nvshmemi_is_device_state_ready ||
        ...
        for (auto it = registered_device_states.cbegin(); it != registered_device_states.cend();
            ...
            status = hipMemcpy((it->first), (void *)device_state,
                                sizeof(nvshmemi_device_host_state_t), hipMemcpyHostToDevice);
            if (status) break;
        }
    ...
    if (nvshmemi_options.IB_ENABLE_IBGDA) {
        ...
        for (auto it = registered_transport_device_states.cbegin();
            ...
            status = hipMemcpy((it->first), (void *)ibgda_device_state,
                                sizeof(nvshmemi_ibgda_device_state_t), hipMemcpyHostToDevice);
            if (status) break;
        }
    }
#endif
...
static void nvshmemi_detect_nvls_support(nvshmemi_state_t *state) {
    ...
    hipDevice_t current_dev;
    CUDA_RUNTIME_CHECK(hipGetDevice(&cuda_dev));
    status = hipDeviceGet(&current_dev, cuda_dev);
    if (status != hipSuccess) {
        WARN("hipDeviceGet failed\n");
        return;
    }
    ...
    status = 
        hipDeviceGetAttribute(
            &mc_support, static_cast<hipDeviceAttribute_t>(CU_DEVICE_ATTRIBUTE_MULTICAST_SUPPORTED),
            current_dev);
    if (status != hipSuccess) {
        WARN("hipDeviceGetAttribute failed\n");
        return;
    }
#endif
    ...
    if (state->heap_obj != nullptr &&
        ...
        WARN("Unsupported heap kind for NVLS. Supported are: hipMemCreate\n");
        return;
    }
...
int nvshmemi_get_cucontext(nvshmemi_state_t *state) {
    ...
    CUCHECK(nvshmemi_cuda_syms, hipInit(0));

    //CUDA_RUNTIME_CHECK(hipSetDevice(nvshmemi_boot_handle.pg_rank));
    status = hipCtxGetDevice(&cudevice);
    if (status || nvshmemi_options.BOOTSTRAP_TWO_STAGE) {
        if (nvshmemi_options.BOOTSTRAP_TWO_STAGE) {
            ...
            TRACE(NVSHMEM_INIT, "GPU not selected, hipCtxGetDevice failed, err: %d", status);

        status = NVSHMEMX_ERROR_GPU_NOT_SELECTED;
        goto out;
    } else {
        hipError_t cres = hipCtxSynchronize();
        if (cres) {
            CUCHECK(nvshmemi_cuda_syms, hipDevicePrimaryCtxRetain(&state->cucontext, cudevice));
            CUCHECK(nvshmemi_cuda_syms, hipCtxSetCurrent(state->cucontext));
            INFO(NVSHMEM_INIT, "retained primary context for device: %d", cudevice);
        } else {
            INFO(NVSHMEM_INIT,
                 "[%d] nvshmemi_get_cucontext->hipCtxSynchronize->hipSuccess) my_stream %p",
                 state->mype, state->my_stream);
            CUCHECK(nvshmemi_cuda_syms, hipCtxGetCurrent(&state->cucontext));
            INFO(NVSHMEM_INIT,
        ...
        if (nvshmemi_cuda_driver_version >= 12010) {
            unsigned int flags;
            CUASSERTAPIAVAILABLE(nvshmemi_cuda_syms, hipCtxSetFlags);
            CUCHECK(nvshmemi_cuda_syms, hipCtxGetFlags(&flags));
            CUCHECK(nvshmemi_cuda_syms, hipCtxSetFlags(flags | CU_CTX_SYNC_MEMOPS));
        }
#endif
        ...
        // identify device id
        int count;
        hipDevice_t curr_device;
        CUDA_RUNTIME_CHECK(hipGetDeviceCount(&count));

        for (int i = 0; i < count; i++) {
            CUCHECK(nvshmemi_cuda_syms, hipDeviceGet(&curr_device, i));
            if (curr_device == cudevice) {
                state->device_id = i;
                break;
            }
        }
        CUDA_RUNTIME_CHECK(hipDeviceGetStreamPriorityRange(&leastPriority, &greatestPriority));
        CUDA_RUNTIME_CHECK(hipStreamCreateWithPriority(&state->my_stream, hipStreamNonBlocking,
                                                        greatestPriority));
...
int nvshmemi_setup_stream_priorities(nvshmemi_state_t *state) {
    ...
    CUDA_RUNTIME_CHECK(hipDeviceGetStreamPriorityRange(&leastPriority, &greatestPriority));
    CUDA_RUNTIME_CHECK(
        hipStreamCreateWithPriority(&state->my_stream, hipStreamNonBlocking, greatestPriority));
...
int nvshmemi_teardown_handles(nvshmemi_state_t *state) {
    ...
    for (int i = 0; i < MAX_PEER_STREAMS; i++) {
        CUDA_RUNTIME_CHECK_GOTO(hipStreamDestroy(state->custreams[i]), status, out);
        CUDA_RUNTIME_CHECK_GOTO(hipEventDestroy(state->cuevents[i]), status, out);
    }
out:
    return status;
}
...
static int nvshmemi_setup_nvshmem_handles(nvshmemi_state_t *state) {
    ...
    CUDA_RUNTIME_CHECK(hipDeviceGetAttribute(
        &dev_attr, hipDeviceAttributeCanUseHostPointerForRegisteredMem, state->device_id));
    state->host_memory_registration_supported =
        dev_attr & hipDeviceAttributeCanUseHostPointerForRegisteredMem;
...
static int nvshmemi_setup_cuda_handles(nvshmemi_state_t *state) {
    int status = 0;
    state->custreams = (hipStream_t *)malloc(MAX_PEER_STREAMS * sizeof(hipStream_t));
    state->cuevents = (hipEvent_t *)malloc(MAX_PEER_STREAMS * sizeof(hipEvent_t));
    state->active_internal_streams = (bool *)calloc(MAX_PEER_STREAMS, sizeof(bool));
    int leastPriority, greatestPriority;
    CUDA_RUNTIME_CHECK(hipDeviceGetStreamPriorityRange(&leastPriority, &greatestPriority));
    for (int i = 0; i < MAX_PEER_STREAMS; i++) {
        CUDA_RUNTIME_CHECK_GOTO(hipStreamCreateWithPriority(
                                    &state->custreams[i], hipStreamNonBlocking, greatestPriority),
                                status, out);
        CUDA_RUNTIME_CHECK_GOTO(
            hipEventCreateWithFlags(&state->cuevents[i], hipEventDisableTiming), status, out);
    }
out:
    return status;
}
...
static int nvshmemi_setup_limited_mpg_support() {
    ...
    CUDA_RUNTIME_CHECK(hipEventCreateWithFlags(&nvshmemi_state->mps_event,
                                       hipEventDisableTiming | hipEventInterprocess));
    CUDA_RUNTIME_CHECK(hipIpcGetEventHandle(
        (hipIpcEventHandle_t *)&shm->event_handle[nvshmemi_team_same_gpu.my_pe],
    ...
    for (int i = 0; i < nvshmemi_team_same_gpu.size; i++) {
        ...
        CUDA_RUNTIME_CHECK(
            hipIpcOpenEventHandle(&event, *(hipIpcEventHandle_t *)&shm->event_handle[i]));
...
static int nvshmemi_mpg_finalize() {
    ...
    CUDA_RUNTIME_CHECK(hipEventDestroy(nvshmemi_state->mps_event));
...
static void nvshmemi_query_cuda_attributes() {
    ...
    if (status != hipSuccess) {
        fprintf(stderr, "hipCtxGetDevice failed \n");
        exit(-1);
    }
    ...
    status = hipDeviceGetAttribute(
        ...
        (hipDeviceAttribute_t)CU_DEVICE_ATTRIBUTE_CAN_USE_64_BIT_STREAM_MEM_OPS, device);
    if (status != hipSuccess) {
        nvshmemi_can_use_cuda_64_bit_stream_memops = false;
    ...
    status = hipDeviceGetAttribute(
        ...
        (hipDeviceAttribute_t)CU_DEVICE_ATTRIBUTE_CAN_FLUSH_REMOTE_WRITES, device);
    if (status != hipSuccess) {
        nvshmemi_can_flush_remote_writes = false;
        CUDA_RUNTIME_CHECK(hipGetLastError());
    }
...
int nvshmemi_common_init(nvshmemi_state_t *state) {
    ...
    CUDA_RUNTIME_CHECK(hipDriverGetVersion(&nvshmemi_cuda_driver_version));
...
void nvshmemid_hostlib_finalize(void *device_ctx, void *transport_device_ctx) {
    ...
    if (nvshmemi_device_state.nvshmemi_is_nvshmem_initialized) {
        ...
        if (nvshmemi_device_state.peer_heap_base_p2p)
            CUDA_RUNTIME_CHECK(hipFree(nvshmemi_device_state.peer_heap_base_p2p));
        if (nvshmemi_device_state.peer_heap_base_remote)
            CUDA_RUNTIME_CHECK(hipFree(nvshmemi_device_state.peer_heap_base_remote));
        if (nvshmemi_device_state.test_wait_any_start_idx_ptr)
            CUDA_RUNTIME_CHECK(hipFree(nvshmemi_device_state.test_wait_any_start_idx_ptr));
...
int nvshmemi_init_device_state(nvshmemi_state_t *state) {
    int status = hipSuccess;
    int warp_size = 0;

    CUDA_RUNTIME_CHECK_GOTO(
        hipDeviceGetAttribute(&warp_size, hipDeviceAttributeWarpSize, state->device_id), status, out);
    ...
    CUDA_RUNTIME_CHECK_GOTO(hipMalloc(&heap_base_array_dptr, (state->npes) * sizeof(void *)),
                            status, out);
    CUDA_RUNTIME_CHECK_GOTO(
        hipMalloc(&heap_base_actual_array_dptr, (state->npes) * sizeof(void *)), status, out);

    status = set_job_connectivity(state);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "set_job_connectivity failed \n");

    CUDA_RUNTIME_CHECK_GOTO(
        hipMemcpyAsync(heap_base_array_dptr, (const void *)state->heap_obj->get_local_pe_base(),
                        sizeof(void *) * state->npes, hipMemcpyHostToDevice, state->my_stream),
        status, out);
    CUDA_RUNTIME_CHECK_GOTO(
        hipMemcpyAsync(heap_base_actual_array_dptr,
                        (const void *)state->heap_obj->get_remote_pe_base(),
                        sizeof(void *) * state->npes, hipMemcpyHostToDevice, state->my_stream),
        status, out);

    CUDA_RUNTIME_CHECK_GOTO(hipStreamSynchronize(state->my_stream), status, out);
    ...
    INFO(NVSHMEM_INIT,
         "[%d] status %d hipErrorInvalidValue %d hipErrorInvalidSymbol %d "
         "hipErrorInvalidMemcpyDirection %d hipErrorNoBinaryForGpu %d",
         state->mype, status, hipErrorInvalidValue, hipErrorInvalidSymbol,
         hipErrorInvalidMemcpyDirection, hipErrorNoBinaryForGpu);
    ...
    CUDA_RUNTIME_CHECK(
        hipMalloc((void **)&test_wait_any_start_idx_ptr, sizeof(unsigned long long)));
    CUDA_RUNTIME_CHECK(
        hipMemset((void *)test_wait_any_start_idx_ptr, 0, sizeof(unsigned long long)));
...
out:
    if (status) {
        if (heap_base_array_dptr) CUDA_RUNTIME_CHECK(hipFree(heap_base_array_dptr));
        if (heap_base_actual_array_dptr) CUDA_RUNTIME_CHECK(hipFree(heap_base_actual_array_dptr));
        if (test_wait_any_start_idx_ptr) CUDA_RUNTIME_CHECK(hipFree(test_wait_any_start_idx_ptr));
    }
    return status;
}

int nvshmemx_cumodule_init(hipModule_t module) {
    int status = 0;
    hipDeviceptr_t dptr, transport_dptr = 0;
    size_t size;
    nvshmemi_version_t module_nvshmem_version;

    CUCHECKGOTO(nvshmemi_cuda_syms,
                hipModuleGetGlobal(&dptr, &size, module, "nvshmemi_device_lib_version_d"), status,
                out);
    CUDA_RUNTIME_CHECK(hipMemcpy((void *)&module_nvshmem_version, (const void *)dptr, size,
                                  hipMemcpyDeviceToHost));
    if (nvshmemi_is_version_compatible(nvshmemi_host_lib_version, module_nvshmem_version) != 0) {
        printf("NVSHMEM version in hipModule_t does not match with NVSHMEM host library version\n");
        return 1;
    }

    CUCHECKGOTO(nvshmemi_cuda_syms,
                hipModuleGetGlobal(&dptr, &size, module, "nvshmemi_device_state_d"), status, out);
#ifdef NVSHMEM_IBGDA_SUPPORT
    CUCHECKIGNORE(nvshmemi_cuda_syms, hipModuleGetGlobal(&transport_dptr, &size, module,
    ...
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipMemcpyFromSymbol failed\n");
out:
    return status;
}

int nvshmemx_cumodule_finalize(hipModule_t module) {
    int status = 0;
    hipDeviceptr_t dptr, transport_dptr = 0;
    size_t size;
