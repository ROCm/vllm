...
#include "p2p.h"
#include <hip/hip_runtime.h>                             // for cudaD...
#if !__HIP__
...
int nvshmemt_p2p_can_reach_peer(int *access, struct nvshmem_transport_pe_info *peer_info,
    ...
    status = hipDeviceCanAccessPeer(&p2p_connected, p2p_state->device_id, peer_devid);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipDeviceCanAccessPeer failed \n");

    if (p2p_connected) {
        *access = NVSHMEM_TRANSPORT_CAP_MAP | NVSHMEM_TRANSPORT_CAP_MAP_GPU_ST |
                  NVSHMEM_TRANSPORT_CAP_MAP_GPU_LD;
        status = hipDeviceGetP2PAttribute(&atomics_supported, hipDevP2PAttrNativeAtomicSupported,
                                           p2p_state->device_id, peer_devid);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                              "hipDeviceGetP2PAttribute failed \n");
...
int nvshmemt_p2p_init(nvshmem_transport_t *t) {
    ...
    status = hipCtxGetDevice(&p2p_state->cudevice);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipCtxGetDevice failed \n");

    p2p_state->hostHash = getHostHash();

    status = hipGetDeviceCount(&p2p_state->ndev);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipGetDeviceCount failed \n");

    p2p_state->cudev = (hipDevice_t *)malloc(sizeof(hipDevice_t) * p2p_state->ndev);
    ...
    for (int i = 0; i < p2p_state->ndev; i++) {
        status = hipDeviceGet(&p2p_state->cudev[i], i);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                              "hipDeviceGet failed \n");
        p2p_state->devid[i] = i;
