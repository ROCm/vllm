...
#include <hip/hip_runtime.h>                    // for hipEventRecord, cudaS...
...
void nvshmemi_quiesce_internal_streams(hipStream_t cstrm); /* implemented in quiet.cpp */
...
void nvshmemx_barrier_all_on_stream(hipStream_t stream) {
#if !__HIP__
...
int nvshmemx_barrier_on_stream(nvshmem_team_t team, hipStream_t stream) {
#if !__HIP__
    NVTX_FUNC_RANGE_IN_GROUP(COLL);
#endif
...
void nvshmemxi_sync_all_on_stream(hipStream_t stream) {
    if (nvshmemi_is_limited_mpg_run) {
...
void nvshmemx_sync_all_on_stream(hipStream_t stream) {
#if !__HIP__
    NVTX_FUNC_RANGE_IN_GROUP(COLL);
#endif
    nvshmemi_check_state_and_init();
...
int nvshmemx_team_sync_on_stream(nvshmem_team_t team, hipStream_t stream) {
#if !__HIP__
    NVTX_FUNC_RANGE_IN_GROUP(COLL);
#endif
    NVSHMEMI_CHECK_INIT_STATUS();
