...
#include <hip/hip_runtime.h>                                                  // for cudaDevice...
...
static int get_cuda_bus_id(int cuda_dev, char *bus_id) {
    ...
    err = hipDeviceGetPCIBusId(bus_id, MAX_BUSID_SIZE, cuda_dev);
    if (err != hipSuccess) {
        NVSHMEMI_ERROR_PRINT("hipDeviceGetPCIBusId failed with error: %d \n", err);
...
int nvshmemi_get_pcie_attrs(pcie_id_t *pcie_id, int devid) {
    ...
    status = hipGetDeviceProperties(&prop, devid);
...
int nvshmemi_detect_same_device(nvshmemi_state_t *state) {
    ...
    hipDeviceProp_t prop;
    hipGetDeviceProperties(&prop, state->device_id);
