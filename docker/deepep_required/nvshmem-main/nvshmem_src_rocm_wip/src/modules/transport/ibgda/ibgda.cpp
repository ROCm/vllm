...
#include <hip/hip_runtime.h>                                        // for hipSuccess, hipDevice_t, CUd...
#include <hip/hip_runtime.h>                                // for hipFree, hipMalloc, cudaM...
...
int nvshmemt_ibgda_get_mem_handle(nvshmem_mem_handle_t *mem_handle,
    ...
    if (local_only) {
        ...
        hipPointerAttribute_t buf_attributes;

        status = hipPointerGetAttributes(&buf_attributes, buf);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                              "hipPointerGetAttributes failed.\n");

        status = hipMalloc((void **)&device_mhandle_d, sizeof(*device_mhandle_d));
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                              "hipMalloc failed.\n");

        device_mhandle_h->start = (uint64_t)buf;
        device_mhandle_h->end = (uint64_t)buf + length - 1;
        device_mhandle_h->is_sysmem_scope = (buf_attributes.type != hipMemoryTypeDevice);
        ...
        status = hipMemcpyAsync((void *)device_mhandle_d, (const void *)device_mhandle_h,
                                 sizeof(*device_mhandle_d), hipMemcpyHostToDevice,
        ...
        if (ibgda_device_local_only_mhandles.empty()) {
            ...
            status = hipMemcpyAsync(mhandle_gpu_ptr, (const void *)&device_mhandle_d,
                                     sizeof(device_mhandle_d), hipMemcpyHostToDevice,
                                     ibgda_state->my_stream);
            NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
        ...
        if (ibgda_device_lkeys_d) {
            status = hipFree(ibgda_device_lkeys_d);
            NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                                  "hipFree failed.\n");
        ...
        if (num_lkeys > NVSHMEMI_IBGDA_MAX_CONST_LKEYS) {
            ...
            status = hipMalloc(&ibgda_device_lkeys_d, lkeys_array_size);
            NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                                  "hipMalloc failed.\n");

            status = hipMemcpyAsync(ibgda_device_lkeys_d, (const void *)data_ptr, lkeys_array_size,
                                     hipMemcpyHostToDevice, ibgda_state->my_stream);
            NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
    ...
    status = hipStreamSynchronize(ibgda_state->my_stream);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "stream synchronize failed.\n");

out:
    if (status) {
        if (device_mhandle_d) hipFree(device_mhandle_d);
        if (did_emplace) {
            if (local_only) {
                // Recoverable
...
static int ibgda_mobject_nic_map(struct ibgda_mem_object *mobject, struct ibv_context *context,
    ...
    if (use_dmabuf && mobject->mem_type == IBGDA_MEM_TYPE_GPU) {
        ...
        CUCHECKGOTO(ibgda_cuda_syms,
                    cuMemGetHandleForAddressRange(&fd, (hipDeviceptr_t)addr, dmabuf_size,
...
static int ibgda_gpu_mem_alloc(struct ibgda_mem_object **pmobject, size_t size, size_t alignment,
    ...
    status = hipMalloc(&ptr, bufsize);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipMalloc failed.\n");

    attr_val = 1;
    status = hipPointerSetAttribute(&attr_val, HIP_POINTER_ATTRIBUTE_SYNC_MEMOPS, (hipDeviceptr_t)ptr);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipPointerSetAttribute failed.\n");

    status = hipMemset(ptr, 0, bufsize);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipMemset failed.\n");
...
out:
    if (status) {
        if (ptr) {
            hipError_t _status = hipFree(ptr);
...
static void ibgda_gpu_mem_free(struct ibgda_mem_object *mobject) {
    ...
    status = hipFree(mobject->base.gpu_ptr);
    CUDA_RUNTIME_ERROR_STRING((hipError_t)status);
...
static int ibgda_host_mem_alloc(struct ibgda_mem_object **pmobject, size_t size, size_t alignment,
    ...
    if (gpu_mapping) {
        status = hipHostRegister(ptr, size, hipHostRegisterPortable | hipHostRegisterMapped);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                              "hipHostRegister failed.\n");
        did_host_reg = true;

        status = hipHostGetDevicePointer(&gpu_ptr, ptr, 0);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
...
out:
    if (status) {
        if (did_host_reg) {
            hipError_t _status = hipHostUnregister(ptr);
            CUDA_RUNTIME_ERROR_STRING(_status);
        }
        if (ptr) free(ptr);
        if (mobject) free(mobject);
...
static void ibgda_host_mem_free(struct ibgda_mem_object *mobject) {
    ...
    if (mobject->has_gpu_mapping) {
        status = hipHostUnregister(mobject->base.cpu_ptr);
...
static int ibgda_nic_mem_gpu_map(struct ibgda_mem_object **pmobject, struct mlx5dv_devx_uar *uar,
    ...
    status = hipHostRegister(
        uar->reg_addr, size,
        hipHostRegisterPortable | hipHostRegisterMapped | hipHostRegisterIoMemory);
    if (status != hipSuccess) {
        NVSHMEMI_WARN_PRINT(
            "hipHostRegister with IoMemory failed with error=%d. We may need to use a fallback "
            "path.\n",
            status);
        status = NVSHMEMX_ERROR_INTERNAL;
        goto out;
    ...
    status = hipHostGetDevicePointer(&ptr, uar->reg_addr, 0);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipHostGetDevicePointer failed.\n");
...
out:
    if (status) {
        if (did_host_reg) {
            hipError_t _status = hipHostUnregister(uar->reg_addr);
            CUDA_RUNTIME_ERROR_STRING(_status);
        }
        if (mobject) free(mobject);
    }
...
static void ibgda_nic_mem_gpu_unmap(struct ibgda_mem_object *mobject) {
    ...
    status = hipHostUnregister(mobject->uar->reg_addr);
    CUDA_RUNTIME_ERROR_STRING(status);
...
static int ibgda_create_cq_shared_objects(nvshmemt_ibgda_state_t *ibgda_state,
    ...
    status = hipMemset(cq_mobject->base.gpu_ptr, 0xff, cq_mobject->base.size);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                          "hipMemset failed.\n");
...
static int ibgda_setup_gpu_state(nvshmem_transport_t t) {
    ...
    if (num_dct_handles > NVSHMEMI_IBGDA_MAX_CONST_DCTS) {
        ...
        status = hipMalloc(&dct_d, num_dct_non_cache_handles * sizeof(*dct_d));
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                              "dct_d allocation failed.\n");
    }

    status = hipMalloc(&dci_d, num_dci_handles * sizeof(*dci_d));
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_OUT_OF_MEMORY, out, "dci cudaM err.");

    if (num_rc_handles > 0) {
        status = hipMalloc(&rc_d, num_rc_handles * sizeof(*rc_d));
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                              "rc_d cudaM err.\n");
    }

    status = hipMalloc(&cq_d, num_cq_handles * sizeof(*cq_d));
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_OUT_OF_MEMORY, out, "cq cudaM err.");

    status = hipMalloc(&qp_group_switches_d, num_qp_groups * sizeof(*qp_group_switches_d));
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
    ...
    hipMemsetAsync(qp_group_switches_d, 0, num_qp_groups * sizeof(*qp_group_switches_d),
                    ibgda_state->my_stream);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
    ...
    if (num_dct_non_cache_handles > 0) {
        ...
        status = hipMemcpyAsync(dct_d, dct_h_ptr, sizeof(*dct_d) * num_dct_non_cache_handles,
                                 hipMemcpyHostToDevice, ibgda_state->my_stream);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out, "dct copy err.");
    }

    status = hipMemcpyAsync(dci_d, (const void *)dci_h, sizeof(*dci_h) * num_dci_handles,
                             hipMemcpyHostToDevice, ibgda_state->my_stream);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out, "dci copy err.");

    if (num_rc_handles > 0) {
        status = hipMemcpyAsync(rc_d, (const void *)rc_h, sizeof(*rc_h) * num_rc_handles,
                                 hipMemcpyHostToDevice, ibgda_state->my_stream);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out, "rc copy err.");
    }

    status = hipMemcpyAsync(cq_d, (const void *)cq_h, sizeof(*cq_h) * num_cq_handles,
                             hipMemcpyHostToDevice, ibgda_state->my_stream);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out, "cq copy err.");
    ...
    status = hipStreamSynchronize(ibgda_state->my_stream);
    NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out, "stream sync err.");
    /* Post the device state start */

out:
    if (status) {
        if (dci_d) hipFree(dci_d);
        if (dct_d) hipFree(dct_d);
        if (cq_d) hipFree(cq_d);
        if (rc_d) hipFree(rc_d);
        if (qp_group_switches_d) hipFree(qp_group_switches_d);
    }
    if (dci_h) free(dci_h);
    if (dct_h) free(dct_h);
    if (cq_h) free(cq_h);
...
static bool ibgda_cst_is_required(struct ibgda_device *device, hipDevice_t dev_id) {
    bool rval = true;

#if !__HIP__
    int order = 0;
    if (hipDeviceGetAttribute(
                  &order, (hipDeviceAttribute_t)CU_DEVICE_ATTRIBUTE_GPU_DIRECT_RDMA_WRITES_ORDERING,
                  dev_id)) {
...
int nvshmemt_ibgda_connect_endpoints(nvshmem_transport_t t, int *selected_dev_ids,
    ...
    if (hipCtxGetDevice(&gpu_device_id)) {
        NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "hipCtxGetDevice failed.\n");
    }

    if (hipDeviceGetAttribute(&warp_size, hipDeviceAttributeWarpSize, gpu_device_id)) {
        NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "querying warp size failed.");
    }

    if (hipDeviceGetAttribute(&mtpb, hipDeviceAttributeMaxThreadsPerBlock, gpu_device_id)) {
        NVSHMEMI_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out,
                           "query max threads per block fail.");
    }

    if (hipDeviceGetAttribute(&mpc, hipDeviceAttributeMultiprocessorCount, gpu_device_id)) {
    ...
    if (handle->local_only) {
        ...
        if (prev_mhandle_cache) {
            ...
            mhandle_gpu_ptr = (void *)((uintptr_t)prev_mhandle_cache->dev_ptr +
                                       offsetof(nvshmemi_ibgda_device_local_only_mhandle_t, next));
            status =
                hipMemcpyAsync(mhandle_gpu_ptr, (const void *)&prev_mhandle_cache->mhandle.next,
                                sizeof(prev_mhandle_cache->mhandle.next), hipMemcpyHostToDevice,
                                ibgda_state->my_stream);
            NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                                  "Setting local_only_mhandle in GPU memory failed.\n");
        } else {
        ...
        status = hipFree(curr_mhandle_cache->dev_ptr);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                              "hipFree failed.\n");
...
int nvshmemt_ibgda_finalize(nvshmem_transport_t transport) {
    ...
    if (ibgda_device_lkeys_d) {
        hipFree(ibgda_device_lkeys_d);
        ibgda_device_lkeys_d = 0;
    }
    if (ibgda_device_rkeys_d) {
        hipFree(ibgda_device_rkeys_d);
    ...
    if (ibgda_device_state_h) {
        if (ibgda_device_state_h->globalmem.dcts) hipFree(ibgda_device_state_h->globalmem.dcts);
        if (ibgda_device_state_h->globalmem.dcis) hipFree(ibgda_device_state_h->globalmem.dcis);
        if (ibgda_device_state_h->globalmem.cqs) hipFree(ibgda_device_state_h->globalmem.cqs);
        if (ibgda_device_state_h->globalmem.rcs) hipFree(ibgda_device_state_h->globalmem.rcs);
        if (ibgda_device_state_h->globalmem.qp_group_switches)
            hipFree(ibgda_device_state_h->globalmem.qp_group_switches);
    }
...
int nvshmemt_ibgda_add_device_remote_mem_handles(nvshmem_transport_t t, int transport_stride,
    ...
    if (ibgda_device_rkeys_d) {
        status = hipFree(ibgda_device_rkeys_d);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
                              "hipFree failed.\n");
        ibgda_device_rkeys_d = 0;
    }
    ...
    if (num_rkeys > NVSHMEMI_IBGDA_MAX_CONST_RKEYS) {
        ...
        status = hipMalloc(&ibgda_device_rkeys_d, rkeys_array_size);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_OUT_OF_MEMORY, out,
                              "hipMalloc failed.\n");

        status = hipMemcpyAsync(ibgda_device_rkeys_d, (const void *)data_ptr, rkeys_array_size,
                                 hipMemcpyHostToDevice, ibgda_state->my_stream);
        NVSHMEMI_NE_ERROR_JMP(status, hipSuccess, NVSHMEMX_ERROR_INTERNAL, out,
...
out:
    if (status) {
        // Unrecoverable error
        if (ibgda_device_rkeys_d) hipFree(ibgda_device_rkeys_d);
        ibgda_device_rkeys.clear();
    }
    return status;
}
...
int nvshmemt_init(nvshmem_transport_t *t, struct nvshmemi_cuda_fn_table *table, int api_version) {
    ...
    status = hipCtxGetDevice(&gpu_device_id);
    if (status != hipSuccess) {
        status = NVSHMEMX_ERROR_INTERNAL;
        goto out;
    }
 ...
 #if !__HIP__
    status = hipDeviceGetAttribute(&flag, (hipDeviceAttribute_t)CU_DEVICE_ATTRIBUTE_DMA_BUF_SUPPORTED,
                                   gpu_device_id);
    if (status != hipSuccess) {
        status = 0;
        hipGetLastError();
    ...
    status = hipDeviceGetStreamPriorityRange(&lowest_stream_priority, &highest_stream_priority);
    NVSHMEMI_NZ_ERROR_JMP(status, NVSHMEMX_ERROR_INTERNAL, out, "stream priority query failed. \n");
    status = hipStreamCreateWithPriority(&ibgda_state->my_stream, hipStreamNonBlocking,
