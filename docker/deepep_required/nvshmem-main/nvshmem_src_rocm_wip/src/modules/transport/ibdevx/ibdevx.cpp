...
#include <hip/hip_runtime.h>                                            // for hipSuccess, CU_DEVICE...
#include <hip/hip_runtime.h>                                    // for hipGetLastError
...
int nvshmemt_init(nvshmem_transport_t *t, struct nvshmemi_cuda_fn_table *table, int api_version) {
    ...
    *t = transport;
    ...
    status = CUPFN(table, hipCtxGetDevice(&gpu_device_id));
    if (status != hipSuccess) {
        status = NVSHMEMX_ERROR_INTERNAL;
        goto out;
    }
    status = CUPFN(table, hipDeviceGetAttribute(
                              &flag, (hipDeviceAttribute_t)CU_DEVICE_ATTRIBUTE_DMA_BUF_SUPPORTED,
                              gpu_device_id));
    if (status != hipSuccess) {
        status = 0;
        hipGetLastError();
