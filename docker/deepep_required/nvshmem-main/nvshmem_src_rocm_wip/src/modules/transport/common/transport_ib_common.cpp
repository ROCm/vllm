...
#include <hip/hip_runtime.h>                              // for hipDeviceptr_t, CU_MEM_RA...
#include <hip/hip_runtime.h>                      // for hipGetLastError, cuda...
...
#if __HIP__
    if (access("/sys/kernel/mm/memory_peers/amdkfd/version", F_OK) == 0) {
...
int nvshmemt_ib_common_reg_mem_handle(struct nvshmemt_ibv_function_table *ftable, struct ibv_pd *pd,
    ...
    status = hipPointerGetAttributes(&attr, buf);
    ...
    if (ftable->reg_dmabuf_mr != NULL && !host_memory && dmabuf_support) {
        ...
        size_t size_aligned;
        hipDeviceptr_t p;
        p = (hipDeviceptr_t)((uintptr_t)buf & ~(page_size - 1));
        size_aligned =
            ((length + (uintptr_t)buf - (uintptr_t)p + page_size - 1) / page_size) * page_size;

        CUCHECKGOTO(table,
                    cuMemGetHandleForAddressRange(&handle->fd, (hipDeviceptr_t)p, size_aligned,
